...
Процедура ВыполнитьПоискНаСервере()
	...
	Иначе
		ПроцедурыОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не выбран режим поиска!'"),,"ПереключательВыборПоляПоиска");
		Возврат;
	КонецЕсли;
	Если ПереключательВыборПоляПоиска = "ПоТелефону" Тогда
		Выборка = Запрос.Выполнить().Выбрать();
...
Процедура ВладелецНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник." + ИмяВладельца + ".ФормаВыбора", ПараметрыФормы, Элемент);
...
Процедура Поиск(Команда)
	Если ПустаяСтрока(ТекущийСправочник) Тогда
		ПроцедурыОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не выбран источник поиска!'"),,"ТекущийСправочник");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(ПолеПоиска) Тогда
		ПроцедурыОбщегоНазначенияКлиентСервер.СообщитьПользователю(Нстр("ru = 'Не заполнено значение поиска!'"),,"ПолеПоиска");
		Возврат;
	КонецЕсли;
