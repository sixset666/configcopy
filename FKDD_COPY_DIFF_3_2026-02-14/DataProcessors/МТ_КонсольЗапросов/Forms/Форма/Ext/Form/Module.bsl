...
Процедура ПоместитьРедактируемыйЗапрос()
	...
	Если РедактируемыйЗапрос >= 0 Тогда
		...
		Элементы.ТекстЗапроса.ПолучитьГраницыВыделения(КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки,
			КурсорКонецКолонки);
		Элементы.ТекстАлгоритм.ПолучитьГраницыВыделения(КодКурсорНачалоСтроки, КодКурсорНачалоКолонки,
			КодКурсорКонецСтроки, КодКурсорКонецКолонки);
		Запрос_ПоместитьДанныеЗапроса(РедактируемыйЗапрос, стрТекстЗапроса, стрТекстАлгоритм, МетодИсполненияКода,
			...
			КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки,
			КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки);
	КонецЕсли;
...
Процедура ВосстановлениеПозицииРедактирования()
	...
	ТекущаяСтрока = Элементы.ПакетЗапросов.ТекущаяСтрока;
	стДанныеЗапроса = Запрос_ПолучитьДанныеЗапроса(ТекущаяСтрока);
	Элементы.ТекстЗапроса.УстановитьГраницыВыделения(стДанныеЗапроса.КурсорНачалоСтроки,
		стДанныеЗапроса.КурсорНачалоКолонки, стДанныеЗапроса.КурсорКонецСтроки, стДанныеЗапроса.КурсорКонецКолонки);
	Элементы.ТекстАлгоритм.УстановитьГраницыВыделения(стДанныеЗапроса.КодКурсорНачалоСтроки,
		стДанныеЗапроса.КодКурсорНачалоКолонки, стДанныеЗапроса.КодКурсорКонецСтроки,
		стДанныеЗапроса.КодКурсорКонецКолонки);
...
Функция Запрос_ПолучитьДанныеЗапроса(идЗапрос)
	...
	Если идЗапрос = Неопределено Тогда
		Возврат Новый Структура("Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВКонструкторе, КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки",
			"", "", "", 2, Неопределено, Ложь, 1, 1, 1, 1, 1, 1, 1, 1);
	КонецЕсли;

	СтрокаЗапроса = ПакетЗапросов.НайтиПоИдентификатору(идЗапрос);
	Возврат Новый Структура("Имя, Запрос, ТекстКод, МетодИсполненияКода, Параметры, ВременныеТаблицы, ВКонструкторе, КурсорНачалоСтроки, КурсорНачалоКолонки, КурсорКонецСтроки, КурсорКонецКолонки, КодКурсорНачалоСтроки, КодКурсорНачалоКолонки, КодКурсорКонецСтроки, КодКурсорКонецКолонки",
		...
		СтрокаЗапроса.КодКурсорКонецКолонки + 1);
...
Процедура Запрос_ПоместитьДанныеЗапроса(идЗапрос, стрТекстЗапроса, стрТекстКод = Неопределено,
	...
	КодКурсорКонецСтроки = Неопределено, КодКурсорКонецКолонки = Неопределено)
...
&НаСервере
Процедура ПроверитьТекстЗапросаПоВладельцу()
	
	Если НЕ ВременныйТекстЗапроса = "" Тогда
		ТекстЗапроса.УстановитьТекст(ВременныйТекстЗапроса);
	КОнецЕсли;	
	Если ВременныеПараметрыЗапроса.Количество() Тогда
		ПараметрыЗапроса.Загрузить(ВременныеПараметрыЗапроса.Выгрузить());
	КонецЕсли;	
...
Процедура ПриОткрытииЗавершение()
	...
	ПроверитьТекстЗапросаПоВладельцу();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТекстЗапроса") Тогда
		ВременныйТекстЗапроса = Параметры.ТекстЗапроса;
	КонецЕсли;
	
	Если Параметры.Свойство("ПараметрыЗапроса") Тогда
		ВременныеПараметрыЗапроса.Загрузить(Параметры.ПараметрыЗапроса.Выгрузить());
	КонецЕсли;	
...
Процедура ПослеВыбораФайлаДляЗагрузкиПакетаЗапросов(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры = Новый Структура("Продолжение, ИмяФайла",
...
Функция ПолучитьСостояниеФоновогоЗадания()
	...
	ИначеЕсли
	ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Завершено Тогда
		СостояниеЗадания.Состояние = 1;
	ИначеЕсли
	ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		СостояниеЗадания.Состояние = 2;
	ИначеЕсли
	ФоновоеЗадание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	ИмяКнопки = ЭтаФорма.ВладелецФормы.ТекущийЭлемент.Имя;
	
	если ИмяКнопки = "СформироватьЗапросРасшифровки" Тогда
		ТЗапросаВладельца = ЭтаФорма.ВладелецФормы.Объект.Модульрасшифровки;
		ИмяТЗ = "ПараметрыРасшифровки";
	иначе	
		ТЗапросаВладельца = ЭтаФорма.ВладелецФормы.Объект.ТЗапроса;
		имяТЗ = "ПараметрыЗапроса";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТЗапросаВладельца) и ТекстЗапроса.КоличествоСтрок() Тогда
		Ответ = Вопрос("Уже есть заполненный запрос, заменить?",РежимДиалогаВопрос.ДаНетОтмена);
	ИначеЕсли ТекстЗапроса.КоличествоСтрок() Тогда 
		Ответ = Вопрос("Перенести запрос в справочник показателей KPI?",РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		
		Если ИмяКнопки = "СформироватьЗапросРасшифровки" Тогда
			ЭтаФорма.ВладелецФормы.Объект.МодульРасшифровки =  ТекстЗапроса.ПолучитьТекст();	
		Иначе
			ЭтаФорма.ВладелецФормы.Объект.ТЗапроса =  ТекстЗапроса.ПолучитьТекст();
		КонецЕсли;
		
		Если ПараметрыЗапроса.Количество() Тогда
			
			ЭтаФорма.ВладелецФормы[ИмяТЗ].Очистить();

			Для Каждого СтрПараметра Из ПараметрыЗапроса Цикл
				НовыйПараметр = ЭтаФорма.ВладелецФормы[ИмяТЗ].Добавить();
				ЗаполнитьЗначенияСвойств(НовыйПараметр, СтрПараметра);
			КонецЦикла;
		КонецЕсли;	
