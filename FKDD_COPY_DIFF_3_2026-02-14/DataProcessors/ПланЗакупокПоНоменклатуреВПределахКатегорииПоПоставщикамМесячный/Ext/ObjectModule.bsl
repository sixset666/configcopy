...
Процедура ЗаполнитьПлан() Экспорт
	...
	// Определим поставщиков товаров по одному из трёх алгоритмов
	
	Для Каждого ТекущаяСтрока Из ДокументОбъект.Показатели Цикл

		// Пробуем реализовать первый алгоритм - выбираем поставщика по максимальному объёму поставок за предшествующий период
		...
		ТекстЗапроса="ВЫБРАТЬ
		             ...
		             |ИЗ
		             |	РегистрНакопления.ПартииТоваровКомпании.Обороты(&ДатаНачала, &ДатаКонца, , Партия.ХозОперация = &ХозОперация И СкладКомпании.Подразделение=&ПодразделениеКомпании) КАК ПартииТоваровКомпанииОбороты
		             |
		             |ГДЕ
		...
		Иначе
			...
			ГраницаАктуальности=ДокументОбъект.ПолучитьПараметрПлана("Граница актуальности цен");
			...
			Иначе
				...
				Иначе
					...
					Сообщить("Для товара «"+ТекущаяСтрока.ВидАналитикиПланирования1+"» невозможно определить поставщика", СтатусСообщения.ОченьВажное);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
