// Модуль документа Бюджет закупок автомобилей
...
Функция ПолучитьОбязательныеРеквизиты() Экспорт
	...
	Если ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации Тогда
...
Функция ПолучитьСписокХозОпераций() Экспорт
	...
	СписокХозОпераций.Добавить("БюджетЗакупокПоМоделям"   ,"По моделям",Истина);
	СписокХозОпераций.Добавить("БюджетЗакупокПоВариантамКомплектации","По вариантам комплектации",Ложь);	
...
Процедура СформироватьСтрокуПоследнегоЗаполнения(ЭтаФорма, СпособЗаполнения=-1) Экспорт
	...
	ИначеЕсли СпособЗаполнения=2 Тогда		
		...
		Иначе
			...
			Если ХозОперация=Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации И 
				(ДокументОснование.ХозОперация=Справочники.ХозОперации.БюджетПродажПоМоделям ИЛИ ДокументОснование.ХозОперация=Справочники.ХозОперации.БюджетЗакупокПоМоделям) Тогда
				Текст = Текст + Символы.ПС + "Метод распределения суммы закупки категорий:  " + СокрЛП(МетодыРаспределенияКатегорий);
...
//
Функция ПечатьБюджетЗакупокАвтомобилей(ТабДокумент) Экспорт
	
	Макет = ПолучитьМакет("БюджетЗакупокАвтомобилей");
	...
	Для Каждого СтрокаТабличнойЧасти Из ВыборкаТабличнойЧасти Цикл
		...
		ОбластьСтрока.Параметры.МодельНаименование = спПолучитьНаименование(СтрокаТабличнойЧасти.Модель);
		ОбластьСтрока.Параметры.Модель = СтрокаТабличнойЧасти.Модель;
		Если НЕ ПоМоделям Тогда
			ОбластьСтрока.Параметры.ВариантКомплектацииНаименование = спПолучитьНаименование(СтрокаТабличнойЧасти.ВариантКомплектации);
			ОбластьСтрока.Параметры.ВариантКомплектации = СтрокаТабличнойЧасти.ВариантКомплектации;
		КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ОснованиеПоВариантамКомплектации = (Основание.ХозОперация=Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации ИЛИ Основание.ХозОперация=Справочники.ХозОперации.БюджетПродажПоВариантамКомплектации);
	Если ТипЗнч(Основание)=Тип("ДокументСсылка.БюджетПродажАвтомобилей") ИЛИ ТипЗнч(Основание)=Тип("ДокументСсылка.БюджетЗакупокАвтомобилей") Тогда
	...
	// Тип цен поменяем
	Если НЕ ТипЦен.Закупка Тогда
		ТипЦен = обПраво("ОсновнойТипЦенЗакупкиАвтомобилей", ЭтотОбъект.Права,, ЭтотОбъект);
	КонецЕсли;
