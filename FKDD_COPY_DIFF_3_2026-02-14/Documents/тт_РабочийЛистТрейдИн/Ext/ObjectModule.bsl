...
//Перем СписокСтатусов Экспорт;						//
//Перем СписокСостояний Экспорт;						//
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	БТ_РабочийЛистТрейдИнИсторияСостояний.Состояние КАК Состояние
	|ИЗ
	|	РегистрСведений.БТ_РабочийЛистТрейдИнИсторияСостояний КАК БТ_РабочийЛистТрейдИнИсторияСостояний
	|ГДЕ
	|	БТ_РабочийЛистТрейдИнИсторияСостояний.тт_РабочийЛистТрейдИн = &тт_РабочийЛистТрейдИн
	|
	|УПОРЯДОЧИТЬ ПО
	|	БТ_РабочийЛистТрейдИнИсторияСостояний.ДатаИзменения УБЫВ";
	Запрос.УстановитьПараметр("тт_РабочийЛистТрейдИн", Ссылка);
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ТекСостояние = ВыборкаДетальныеЗаписи.Состояние;	
	КонецЦикла;
	...
	Если Состояние <> ТекСостояние Тогда
		...
		НоваяЗапись.тт_РабочийЛистТрейдИн = Ссылка;
...
//СписокСтатусов.Добавить("Trade In");
//СписокСтатусов.Добавить("Комиссия");
//СписокСтатусов.Добавить("Выкуп");

//СписокСостояний	= Новый СписокЗначений();
//СписокСостояний.Добавить("В продаже");
//СписокСостояний.Добавить("Не на складе");
//СписокСостояний.Добавить("Ограничения");
//СписокСостояний.Добавить("Ремонт");
//СписокСостояний.Добавить("Продан");
