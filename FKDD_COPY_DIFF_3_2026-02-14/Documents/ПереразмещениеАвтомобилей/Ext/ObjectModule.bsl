...
Функция ОбработкаРеквизита(Имя,ТекСтрока=Неопределено,ЭтаФорма=Неопределено,ДопПараметры=Неопределено) Экспорт
	...
	Если Имя="Автомобили.Автомобиль" Тогда
		Если (НЕ обЗначениеНеЗаполнено(ТекСтрока.Автомобиль)) И (НЕ обЗначениеНеЗаполнено(СкладКомпании)) Тогда
			Запрос=Новый Запрос;
			...
			Если Выборка.Следующий() Тогда
				Если Выборка.Количество>0 И Выборка.Ячейка.Владелец=СкладКомпании Тогда
					ТекСтрока.Ячейка=Выборка.Ячейка;
				КонецЕсли; 
			КонецЕсли; 
		КонецЕсли; 
		Возврат Истина;
...
Функция ПолучитьСписокХозОпераций() Экспорт
	...
	СписокХозОпераций.Добавить("ПереразмещениеАвтомобилей", "Переразмещение автомобилей", Истина);
...
Функция ПолучитьОбязательныеРеквизиты() Экспорт
	...
	ОбязательныеАвтомобили.Вставить("ЯчейкаНовая",1);
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	Если СкладКомпании.ВидСклада = Перечисления.ВидыСкладов.Ячеистый И Уникальность Тогда
		...
		Для Каждого СтрокаТаблицы Из Автомобили Цикл
			...
			Если СписокНайденныхДублей.НайтиПоЗначению(СтрокаТаблицы) = Неопределено Тогда
				...
				Если НайденныеСтроки.Количество() > 1 Тогда
					...
					КонецЦикла;
					дкДобавитьОшибку(Ошибки,"Таблица < Автомобили > значения колонки < Ячейка старая > не уникальны! Строки: " + Сред(ДублирующиесяСтроки, 2));
				КонецЕсли;
			КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Автомобили Цикл
			...
			Если СписокНайденныхДублей.НайтиПоЗначению(СтрокаТаблицы) = Неопределено Тогда
				СтруктураОтбора.Вставить("ЯчейкаНовая", СтрокаТаблицы.Ячейка);
				НайденныеСтроки = Автомобили.НайтиСтроки(СтруктураОтбора);
				Если НайденныеСтроки.Количество() > 1 Тогда
					// добавим строку в список найденных дублей, что бы не сообщать о ней еще раз
					Результат = Ложь;
					...
					Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
						...
						СписокНайденныхДублей.Добавить(НайденнаяСтрока);
					КонецЦикла;
					дкДобавитьОшибку(Ошибки,"Таблица < Автомобили > значения колонки < Ячейка новая > не уникальны! Строки: " + Сред(ДублирующиесяСтроки, 2));
				КонецЕсли;
			КонецЕсли;
...
//
Функция ПечатьПереразмещениеАвтомобилей(ТабДокумент) Экспорт
	Макет = ПолучитьМакет("ПереразмещениеАвтомобилей");
...
// Процедура - обработчик ввода для сертификации
Процедура ОбработкаЗаполнения(Основание)
	Если НЕ дкОбработкаЗаполнения(ЭтотОбъект, Основание) Тогда Возврат; КонецЕсли;
	Если НЕ обЗначениеНеЗаполнено(ДокументОснование) Тогда
		Если ТипЗнч(ДокументОснование)=Тип("ДокументСсылка.ПриходныйСкладскойОрдерНаАвтомобили") Тогда
			Автомобили.Очистить();
			...
			КонецЦикла; 
		ИначеЕсли ТипЗнч(ДокументОснование)=Тип("ДокументСсылка.ПереразмещениеАвтомобилей") Тогда
			Автомобили.Очистить();
			Для каждого СтрокаТЧ Из ДокументОснование.Автомобили Цикл
				НоваяСтрокаТЧ=Автомобили.Добавить();
				НоваяСтрокаТЧ.Автомобиль=СтрокаТЧ.Автомобиль;
