...
// Модуль документа РасходныйСкладскойОрдерНаАвтомобили
...
Функция ПолучитьСписокХозОпераций() Экспорт
	...
	СписокХозОпераций.Добавить("РасходныйСкладскойОрдерНаАвтомобили", "Расходный складской ордер на автомобили", Истина);
...
//
Функция ПечатьРасходныйСкладскойОрдерНаАвтомобили(ТабДокумент) Экспорт
	Макет = ПолучитьМакет("РасходныйСкладскойОрдерНаАвтомобили");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если Автомобили.Количество() > 0 И СкладКомпании.ВидСклада = Перечисления.ВидыСкладов.Ячеистый Тогда
		...
		Для Каждого СтрАвтомобиль Из Автомобили Цикл
			Отбор.Вставить("Автомобиль", СтрАвтомобиль.Автомобиль);
			МассивЯчеек = ТаблицаОстатков.НайтиСтроки(Отбор);
			Если МассивЯчеек.Количество()>0 Тогда
				СтрАвтомобиль.Ячейка = МассивЯчеек[0].Ячейка;
			КонецЕсли;
		КонецЦикла;
