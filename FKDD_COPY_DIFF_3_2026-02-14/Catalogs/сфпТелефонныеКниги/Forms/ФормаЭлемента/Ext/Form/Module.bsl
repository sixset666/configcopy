...
Процедура ЗаполнитьТаблицуВидовТелефонов()
	...
	НоваяСтрока.Вид				= ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонКонтактный");
	НоваяСтрока.Тип				= 0;
...
Функция ПолучитьДлинуВнутреннихНомеров()
	Возврат Константы.CRM_МаксимальнаяДлинаВнутреннегоНомера.Получить();
...
Функция ПроверитьЗаполнениеТелефоннойКниги()
	...
	Если ПустаяСтрока(Объект.Наименование) Тогда	
		ЕстьОшибки = Истина;
		Сообщить(НСтр("ru='Не заполнено наименование телефонной книги'"));
	КонецЕсли;	
	НайденныеСтроки = Объект.ВидыТелефонов.НайтиСтроки(Новый Структура("Пометка", Истина));
	Если НайденныеСтроки.Количество() = 0 Тогда
		ЕстьОшибки = Истина;
		Сообщить(НСтр("ru='Не выбран ни один вид телефонного номера'"));
	КонецЕсли;
	Если ДеревоТелефоннойКниги.ПолучитьЭлементы().Количество() = 0 Тогда
		ЕстьОшибки = Истина;
		Сообщить(НСтр("ru='Не заполнена телефонная книга'"));
	КонецЕсли;	
...
Процедура ПользователиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		...
		РезультатЗакрытия = ОткрытьФормуМодально("Справочник.Пользователи.ФормаОбъекта", ПараметрыФормы, ЭтаФорма);
		ОбновитьОписаниеПользователя(РезультатЗакрытия, ДополнительныеПараметры)
	КонецЕсли;	
...
Процедура ДобавитьГруппуВТелефоннуюКнигу(Команда)
	...
	Строка = ВвестиСтроку("", НСтр("ru='Введите наименование новой группы'"), , Ложь); 	
	ДобавитьНовуюГруппуВТелефоннуюКнигу(Строка, ДополнительныеПараметры);
...
Процедура ИзменитьИмяГруппыТелефоннойКниги(Команда)
	...
	Строка = ВвестиСтроку(ТД.Наименование, НСтр("ru='Наименование группы'"), , Ложь); 	
	ОбновитьИмяГруппы(Строка, ДополнительныеПараметры);
