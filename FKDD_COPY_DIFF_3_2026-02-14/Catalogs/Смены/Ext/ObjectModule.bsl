	...
	Если Имя="Продолжительность" Тогда
		...
		Иначе
			Если ТекСтрока.НачалоРабочегоВремени <= ТекСтрока.КонецРабочегоВремени Тогда
				...
				Если ТекСтрока.КонецРабочегоВремени = '00010101235959' Тогда //если окончание - конец дня, то продолжительность будет всегда на 1 сек меньше чем надо, поэтому считаем по другому
			...
			Иначе
				...
				Иначе
					Сообщить("Ночной интервал следует разбить на два интервала - до полуночи и после");
					ТекСтрока.КонецРабочегоВремени = '00010101';
				КонецЕсли;
				
				Возврат ОбработкаРеквизита("Продолжительность", ТекСтрока, ЭтаФорма);
			КонецЕсли;
			
		КонецЕсли;
		
	ИначеЕсли Имя="ВремяНачала" Тогда
		Возврат ОбработкаРеквизита("Продолжительность", ТекСтрока, ЭтаФорма);
		
	ИначеЕсли Имя="ВремяОкончания" Тогда
		Возврат ОбработкаРеквизита("Продолжительность", ТекСтрока, ЭтаФорма);
