// Модуль справочника КлассификаторСтранМира
...
// возвращает страну по префиксу ШК. Если страна не найдена, то возвращает неопределено
//
// Параметры:
//  ШтрихКод				– Строка	– Строка штрих-кода.
//
// Возвращаемое значение:
//   Строка		   			– Страна происхождения.
//
Функция ПолучитьСтрануИзШтрихКода(Знач ШтрихКод) Экспорт
	Запрос=Новый Запрос("
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	КлассификаторСтранМираПрефиксыШК.Ссылка КАК СтранаПроисхождения
	|ИЗ
	|	Справочник.КлассификаторСтранМира.ПрефиксыШК КАК КлассификаторСтранМираПрефиксыШК
	|
	|ГДЕ
	|	КлассификаторСтранМираПрефиксыШК.Префикс=&Префикс");
	Запрос.УстановитьПараметр("Префикс",Лев(ШтрихКод,3));
	РезультатЗапросаСтраны=Запрос.Выполнить();
	Если НЕ РезультатЗапросаСтраны.Пустой() Тогда
		Выборка=РезультатЗапросаСтраны.Выбрать(); Выборка.Следующий();
		Возврат Выборка.СтранаПроисхождения;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
КонецФункции // ПолучитьСтрануИзШтрихКода()
