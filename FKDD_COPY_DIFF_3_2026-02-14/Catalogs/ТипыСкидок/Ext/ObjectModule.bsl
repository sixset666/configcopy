// Модуль справочника ТипыСкидокНаценок

////////////////////////////////////////////////////////////////////////////////
// ПЕРЕМЕННЫЕ ОБЪЕКТА

Перем Права Экспорт;	// Переменная объекта - ссылка на коллекцию прав, настроек и переменных окружения
Перем ВалютаНакопительныхСумм Экспорт; // Валюта накопительных сумм
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		ОбязательныеРеквизиты.Вставить("СпособВычисления",1);
	КонецЕсли;
...
// Процедура формирования по умолчанию
Процедура СформироватьНаименованиеПоУмолчанию() Экспорт
	НовоеНаименование="";
	НовоеНаименование=НовоеНаименование+Формат(ЗначениеСкидки,"ЧЦ=15; ЧДЦ=2; ЧН=0.00");
	НовоеНаименование=НовоеНаименование+?(СпособВычисления=Перечисления.СкидкиСпособВычисления.Относительная,"%"," "+СокрЛП(ВалютаНакопительныхСумм));
	НовоеНаименование=НовоеНаименование+?(РучнаяСкидка," Ручная"," Автоматическая");
	Если НЕ РучнаяСкидка Тогда
		НовоеНаименование=НовоеНаименование+?(НакопительнаяСумма>0," на накопительную сумму от "+Формат(НакопительнаяСумма,"ЧЦ=15; ЧДЦ=2; ЧН=0.00")+" "+СокрЛП(ВалютаНакопительныхСумм),"");
		НовоеНаименование=НовоеНаименование+?(СуммаДокумента>0," на сумму документа от "+Формат(СуммаДокумента,"ЧЦ=15; ЧДЦ=2; ЧН=0.00")+" "+СокрЛП(ВалютаНакопительныхСумм),"");
	КонецЕсли; 
	Если ДействуетДляКарточек<>Справочники.Карточки.ПустаяСсылка() Тогда
		НовоеНаименование=НовоеНаименование+" для "+СокрЛП(ДействуетДляКарточек);
	КонецЕсли; 
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ЭтоНовый() И (НЕ ЭтоГруппа)Тогда
		СпособВычисления=Перечисления.СкидкиСпособВычисления.Относительная;
		ВидСкидки=Перечисления.ВидыСкидок.НаДокумент;
		ИспользоватьДляФронтОфиса=Истина;
		Точность=2;
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ)
	Если (НЕ ЭтоГруппа) И (РучнаяСкидка) Тогда
		НакопительнаяСумма=Неопределено; СуммаДокумента=Неопределено;
		ПериодДействияС=Неопределено; ПериодДействияПо=Неопределено;
		ДействуетДляКарточек=Неопределено;
	КонецЕсли; 
