...
// Формирует наименование для рабочего места
Функция ПолучитьНаименованиеРабочегоМеста() Экспорт
	...
	Если ПустаяСтрока(ИмяКомпьютера) Тогда
		...
		Иначе
			ПозРазделителя = 0;
			Для Инд=2 По СтрДлина(Наименование) Цикл
				...
				КонецЕсли;
			КонецЦикла;
			Если ПозРазделителя > 1 Тогда
				ИмяКомпьютера = Лев(Наименование, ПозРазделителя - 1);
			Иначе
				ИмяКомпьютера = СокрЛП(Наименование);
			КонецЕсли;
		КонецЕсли;	
	КонецЕсли;
	Результат = СокрЛП(ИмяКомпьютера) + ";";
	Если Пользователь.Пустая() Тогда
		Результат = Результат + " (Любой пользователь)";
	Иначе
		Результат = Результат + " (" + СокрЛП(Пользователь.Код) + ")";
	КонецЕсли;
