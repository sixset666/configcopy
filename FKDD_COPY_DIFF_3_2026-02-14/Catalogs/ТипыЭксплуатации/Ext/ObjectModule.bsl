...
// По способу аморт-ии и аморт. группе генерит наименование
Функция ПолучитьНаименование() Экспорт
	Наимен = "";
	Если Консервирование Тогда
		Наимен = Наимен + "Консервирование";
	Иначе
		Если СпособНачисленияАмортизации.Пустая() Тогда
			Возврат Наимен;
		Иначе
			Наимен = Строка(СпособНачисленияАмортизации);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		ОбязательныеРеквизиты.Вставить("Консервирование", 1);
		Если НЕ Консервирование Тогда
			ОбязательныеРеквизиты.Вставить("СпособНачисленияАмортизации", 1);
		КонецЕсли;
	КонецЕсли;
