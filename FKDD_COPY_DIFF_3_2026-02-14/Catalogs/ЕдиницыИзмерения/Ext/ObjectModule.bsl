// Модуль справочника ЕдиницыИзмерения
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	ОбязательныеРеквизиты.Вставить("ЕдиницаПоКлассификатору",1);
	ОбязательныеРеквизиты.Вставить("Коэффициент",1);
...
Процедура СформироватьНаименование() Экспорт
	Наименование = СокрЛП(ЕдиницаПоКлассификатору.Наименование);
	Если Коэффициент <> 1 Тогда
		Наименование = ""+Наименование+" "+Коэффициент;
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если Основание = Неопределено Тогда
		Если ТипЗнч(Владелец)=Тип("СправочникСсылка.Номенклатура") Тогда
			Если Владелец.ТипНоменклатуры.ИспользованиеЕдиницИзмерения<>2 Тогда
			...
			Если НЕ Владелец.ТипНоменклатуры.Весовой Тогда Точность = 0; КонецЕсли;	
		ИначеЕсли  ТипЗнч(Владелец)=Тип("СправочникСсылка.ТипыНоменклатуры") Тогда
			...
			// Установим точность для всех видов штучных товаров
			Если НЕ Владелец.Весовой Тогда Точность = 0; КонецЕсли;	
		Иначе Возврат;
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ТипЗнч(Владелец)=Тип("СправочникСсылка.Номенклатура") Тогда
		уоУстановитьМоментИзменения(Владелец,ТекущаяДата());
	КонецЕсли;
