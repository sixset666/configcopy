...
Процедура ПриОткрытии(Отказ)    
	...
	Элементы.GoodsType.СписокВыбора.ЗагрузитьЗначения(ns_ХоббиИОтдыхФормы.ПолучитьСписокПодкатегорий(Category));  
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	ns_ОбщегоНазначенияФормы.ОбработкаВыбора(Элементы, ВыбранноеЗначение, ИсточникВыбора, "ns_ХоббиИОтдых"); 
КонецПроцедуры
...
Процедура УстановитьВидимостьРеквизитовЗаполнения()
	Если НЕ ЗначениеЗаполнено(Category) Тогда
		...
		Элементы.VehicleType.Видимость = Ложь;
	Иначе
		Элементы.GoodsType.Видимость = ns_ХоббиИОтдыхВалидация.УстановитьВидимость_GoodsType(Category);
		Элементы.VehicleType.Видимость = ns_ХоббиИОтдыхВалидация.УстановитьВидимость_VehicleType(Category);
	КонецЕсли;
КонецПроцедуры
...
Процедура СброситьРеквизитыЗаполнения(ОчищатьCategory = Ложь, ОчищатьGoodsType = Ложь)
	...
	ConditionХоббиИОтдых = Неопределено; 
	ШаблонЗаголовка = Неопределено;
	ШаблонОбъявления = Неопределено;
	VehicleType = Неопределено;
КонецПроцедуры
	...
	Попытка
		...
		Для каждого СтрокаСписка Из ВыделенныеСтроки Цикл
			Если СтрокаСписка.ЭтоГруппа Тогда
				...
				|ГДЕ
				|	ns_ХоббиИОтдых.Родитель В ИЕРАРХИИ(&Родитель)
				|	И ns_ХоббиИОтдых.ЭтоГруппа = ЛОЖЬ";
...
Процедура УстановитьЗначениеЭлементу(Ссылка, ЗначенияСопоставленныхРеквизитов)
	...
	ns_ОбщегоНазначенияВызовСервера.УстановитьНовоеЗначениеРеквизита(Изменен, ТекущийОбъект, "Condition", ConditionХоббиИОтдых);
	Если ЗначениеЗаполнено(ТекущийОбъект.Category) И ТекущийОбъект.Category = ns_ХоббиИОтдыхПовтИсп.ПолучитьВелосипеды() Тогда
...
Процедура УстановитьВыгружать(Команда)
	ns_ОбщегоНазначенияФормы.УстановитьВыгружать(Элементы, Истина, "ns_ХоббиИОтдых");
КонецПроцедуры

&НаКлиенте
Процедура СнятьВыгружать(Команда)
	ns_ОбщегоНазначенияФормы.УстановитьВыгружать(Элементы, Ложь, "ns_ХоббиИОтдых");
КонецПроцедуры
...
Процедура ОбновитьСписокНаСервере()
	...
	Список.ТекстЗапроса = ns_Переопределяемый.ПолучитьТекстЗапроса_ХоббиИОтдых(Склад);
...
Процедура СнятьБезОстатковСВыгрузкиНаСервере()
	ns_АвитоВалидация.СнятьБезОстатковСВыгрузки("ns_ХоббиИОтдых");
КонецПроцедуры
