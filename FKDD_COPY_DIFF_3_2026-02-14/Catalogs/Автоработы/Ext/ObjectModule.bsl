...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		...
		ОбязательныеНормыВремени=Новый Структура();
		ОбязательныеНормыВремени.Вставить("Модель", 3);
		ОбязательныеНормыВремени.Вставить("ВариантКомплектации", 2);
		ОбязательныеНормыВремени.Вставить("ВремяВыполнения", 1);
		ОбязательныеРеквизиты.Вставить("НормыВремени", ОбязательныеНормыВремени);
	КонецЕсли;
...
Функция ОбработкаРеквизита(Имя, ТекСтрока = Неопределено, ЭтаФорма = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Имя = "НормыВремени.Модель" Тогда
		Если ЗначениеЗаполнено(ТекСтрока.ВариантКомплектации) Тогда
			Если НЕ ТекСтрока.ВариантКомплектации.Владелец = ТекСтрока.Модель Тогда
				ТекСтрока.ВариантКомплектации = Справочники.Модели.ПустаяСсылка();
			КонецЕсли;
		КонецЕсли;
		Возврат Истина;
	ИначеЕсли Имя = "НормыВремени.ВариантКомплектации" Тогда
		Если ЗначениеЗаполнено(ТекСтрока.ВариантКомплектации) Тогда
			НоваяМодель = ТекСтрока.ВариантКомплектации.Владелец;
			Если НЕ НоваяМодель = ТекСтрока.Модель Тогда
				ТекСтрока.Модель = НоваяМодель;
			КонецЕсли;
		КонецЕсли;
		Возврат Истина;
	Иначе
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ЭтоНовый() Тогда
		Номенклатура=ЭтотОбъект.Родитель.Номенклатура;
		Если (НЕ ЭтоГруппа) И (обЗначениеНеЗаполнено(Номенклатура)) Тогда
			Номенклатура=обПраво("АвтоработаПоУмолчанию",Права);
		КонецЕсли; 
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоГруппа Тогда
		Если НЕ ЭтоДополнительноеПредложение И НЕ Ссылка.Пустая() Тогда
			...
			Набор.Отбор.ДополнительноеПредложение.Установить(Ссылка);
...
Процедура ПриЗаписи(Отказ)
	...
	Если НЕ Отказ Тогда
		Если НЕ Ссылка.Пустая() ИЛИ НЕ ПометкаУдаления Тогда
			Набор = РегистрыСведений.ПрименяемостьДополнительныхПредложенийПоМоделям.СоздатьНаборЗаписей();
			Набор.Отбор.ДополнительноеПредложение.Установить(Ссылка);
			Набор.Прочитать();
			
			Если Набор.Количество() = 0 Тогда
				Набор.Отбор.Модель.Установить(Справочники.Модели.ПустаяСсылка());
				Набор.Отбор.guid.Установить("");
				
				ПланыОбмена.ОбменСМобильнымУстройством.ЗарегистрироватьИзмененияДляУзла(Набор);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
