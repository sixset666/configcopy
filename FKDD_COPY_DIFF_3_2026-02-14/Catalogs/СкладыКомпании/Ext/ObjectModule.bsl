// Модуль справочника СкладыКомпании

////////////////////////////////////////////////////////////////////////////////
// ПЕРЕМЕННЫЕ ОБЪЕКТА

Перем Права Экспорт; // Переменная объекта - ссылка на коллекцию прав, настроек и переменных окружения
Перем ДопРеквизиты Экспорт; // Структура для проверки заполненности параметров ордерного склада
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		...
		ОбязательныеРеквизиты.Вставить("ВидСклада",1);
		Если Розничный Тогда
			ОбязательныеРеквизиты.Вставить("ТипЦенРозничнойТорговли",1);
			ОбязательныеРеквизиты.Вставить("НомерОтделаДляККМ",1);
		КонецЕсли; 
		Если НЕ ВидСклада = Перечисления.ВидыСкладов.Обычный Тогда
			ОбязательныеРеквизиты.Вставить("УчетЕдиницИзмерения",1);
		КонецЕсли; 
	КонецЕсли;
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	ФлагУчастияВДвижениях = спУчастиеВДвижениях("ОстаткиТоваровОрдерныйСклад", "СкладКомпании",ЭтотОбъект.Ссылка);
	РезультатВидаСклада = Ложь;
	//Проверяем возможное изменение вида склада
	СтарыйВидСклада = ЭтотОбъект.Ссылка.ВидСклада;
	Если СтарыйВидСклада = ВидСклада Тогда
		РезультатВидаСклада = Истина;	
	Иначе	
		Если ФлагУчастияВДвижениях Тогда
			Если ВидСклада = Перечисления.ВидыСкладов.Ячеистый Тогда
				РезультатВидаСклада = Истина;
			ИначеЕсли (ВидСклада = Перечисления.ВидыСкладов.Ордерный)
					И (СтарыйВидСклада = Перечисления.ВидыСкладов.Обычный)Тогда
					РезультатВидаСклада = Истина;
			КонецЕсли;
			Если НЕ РезультатВидаСклада Тогда
				Ошибки = Ошибки +"По данному складу есть движения регистра ""Остатки товаров на ордерном складе""! Нельзя изменять вид склада!"+Символы.ПС;
			КонецЕсли;
		Иначе
			РезультатВидаСклада = Истина;	
		КонецЕсли;
	КонецЕсли;
	РезультатВидаУчетаЕдиницИзмерения = Ложь;
	//Проверяем возможное изменение вида склада
	Если (НЕ (УчетЕдиницИзмерения = ЭтотОбъект.Ссылка.УчетЕдиницИзмерения)
	   И ФлагУчастияВДвижениях) Тогда
		Ошибки = Ошибки +"По данному складу есть движения регистра ""Остатки товаров на ордерном складе""! Нельзя изменять вид учета единиц измерения!"+Символы.ПС;   
   	Иначе
	    РезультатВидаУчетаЕдиницИзмерения = Истина;
	КонецЕсли;
	Результат = РезультатСтандарт И РезультатВидаСклада И РезультатВидаУчетаЕдиницИзмерения;
	Возврат Результат
...
//
Функция ЕстьЗаполненныеЯчейки() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЯчейкиХранения.Представление
	               |ИЗ
	               |	Справочник.ЯчейкиХранения КАК ЯчейкиХранения
	               |ГДЕ
	               |	ЯчейкиХранения.Владелец = &Ссылка
...
Процедура ПередЗаписью(Отказ)
	Если ВидСклада = Перечисления.ВидыСкладов.Ячеистый Тогда
		// дополнительно проверим параметры ордерного склада
		спПередЗаписью(ЭтотОбъект, Отказ, ДопРеквизиты);
	Иначе
...
// дополнительные реквизиты для проверки в случае ордерного склада
ДопРеквизиты=Новый Структура();
//ДопРеквизиты.Вставить("УчетЕдиницИзмерения",1);
ДопРеквизиты.Вставить("ДлинаСклада",1);
ДопРеквизиты.Вставить("ШиринаСклада",1);
ДопРеквизиты.Вставить("ЧислоУровней",1);
ДопРеквизиты.Вставить("ДлинаЯчейкиПоУмолчанию",1);
ДопРеквизиты.Вставить("ШиринаЯчейкиПоУмолчанию",1);
