...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента = Истина, ДляГруппы = Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		ОбязательныеРеквизиты.Вставить("КратностьБонусов",1);
		ОбязательныеРеквизиты.Вставить("ВалютаБонуса",1);
		
		ОбязательныеРеквизитыТЧ = Новый Структура;
		ОбязательныеРеквизитыТЧ.Вставить("Группа", 3);
		
		ОбязательныеРеквизиты.Вставить("ПравилаНачисленияБонусов" , ОбязательныеРеквизитыТЧ);
		ОбязательныеРеквизиты.Вставить("ПравилаСписанияБонусов"   , ОбязательныеРеквизитыТЧ);
	КонецЕсли;
...
Функция ДоступностьИзменения(БлокироватьРеквизиты=Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	БонусныеБаллы.БонуснаяКарта.БонуснаяПрограмма
	|ИЗ
	|	РегистрНакопления.БонусныеБаллы КАК БонусныеБаллы
	|ГДЕ
	|	БонусныеБаллы.БонуснаяКарта.БонуснаяПрограмма = &БонуснаяПрограмма";
	Запрос.УстановитьПараметр("БонуснаяПрограмма", Ссылка);
	
	результат = Запрос.Выполнить();
	
	Если НЕ результат.Пустой() тогда
		БлокироватьРеквизиты = Новый Структура("КратностьБонусов,ВалютаБонуса,МаксимальныйПроцентОплатыБонусами", "КратностьБонусов", "ВалютаБонуса", "МаксимальныйПроцентОплатыБонусами");
		Возврат Ложь;
	КонецЕсли;
