// Модуль справочника Статусы рабочего листа
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
    ...
    Если ДляЭлемента Тогда
		ОбязательныеРеквизиты.Вставить("ВероятностьПродажи",1);
		
		ОбязательныеШаблоны = Новый Структура();
		ОбязательныеШаблоны.Вставить("ШаблонСобытия", 3);
		ОбязательныеРеквизиты.Вставить("ШаблоныСобытий", ОбязательныеШаблоны);
	КонецЕсли;
