// Модуль справочника СтатьиДоходовИРасходов
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если НЕ ЭтоГруппа Тогда
		ОбязательныеРеквизиты.Вставить("ВидДоходаИРасхода", 1);
		ОбязательныеРеквизиты.Вставить("КоррСчет",          1); 
		
		ОбязательныеСтатьиРаспределения = Новый Структура;
		ОбязательныеСтатьиРаспределения.Вставить("СтатьяРаспределения", 3);
		ОбязательныеРеквизиты.Вставить("СтатьиРаспределения", ОбязательныеСтатьиРаспределения);
	КонецЕсли;	
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	// В табличной части "Статьи распределения", в качестве статьи для распределения не может выступать
	// сама (текущая) статья. Иначе произойдет зацикливание.
	Для Каждого Строка Из СтатьиРаспределения Цикл  	
		Если Строка.СтатьяРаспределения=Ссылка Тогда
			Ошибки = Ошибки + "В качестве статьи для распределения нельзя указывать текущую статью. Номер строки: " + Строка.НомерСтроки + Символы.ПС;
			Результат = Ложь;
		ИначеЕсли Строка.СтатьяРаспределения.Пустая() Тогда
			Ошибки = Ошибки + "Не указана статья для распределения. Номер строки: " + Строка.НомерСтроки + Символы.ПС;		
			Результат = Ложь;	
		КонецЕсли;
	КонецЦикла;
