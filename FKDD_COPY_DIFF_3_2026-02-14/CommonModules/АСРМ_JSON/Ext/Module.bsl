...
//
Функция мПрочитатьJSON(Значение, Стандарт = Ложь, ПредставленияСсылок = Ложь) Экспорт 
...
//
Функция мЗаписатьJSON(Значение, Стандарт = Ложь, ПредставленияСсылок = Ложь) Экспорт 
...
Функция jsonЗаписать(Значение, Стандарт, ПредставленияСсылок, ВспомогательныеДанные, Колонки, Смещение, Отступ, Табуляция, ПереносСтроки, ПолноеМаскированиеСимволов, МаскированиеКириллицы)
	...
	Если ВспомогательныеДанные.ПримитивныеТипы.СодержитТип(Тип) Тогда 
		...
		Если (Значение = Null) Тогда Возврат "null"; ИначеЕсли (Значение = Неопределено) Тогда Возврат "undefined"; ИначеЕсли (Тип = Тип("Дата")) Тогда Возврат """" + Формат(Значение, "ДФ=yyyy-MM-ddTHH:mm:ss; ДП=") + "Z""" ИначеЕсли (Тип = Тип("УникальныйИдентификатор")) Тогда Возврат """" + Значение + """"; Иначе Возврат Формат(Значение, "ЧРД=.; ЧН=; ЧГ=; БЛ=false; БИ=true"); КонецЕсли;
		#Иначе
		// Шарипов Артём, 27.10.2016
		...
		Если (Значение = Null) Тогда Возврат "null"; ИначеЕсли (Значение = Неопределено) Тогда Возврат "undefined"; ИначеЕсли (Тип = Тип("Дата")) Тогда Возврат """" + XMLСтрока(Значение) + "Z""" ИначеЕсли (Тип = Тип("УникальныйИдентификатор")) Тогда Возврат """" + XMLСтрока(Значение) + """"; Иначе Возврат XMLСтрока(Значение); КонецЕсли;
		#КонецЕсли
	КонецЕсли;
