...
Функция ВыбратьПолучателя(Объект, МножественныйВыбор = Истина, ВозвращатьСоответствие = Истина, ЗаголовокДляОшибок = "") Экспорт
	...
	Иначе	
		НаборПараметров = "Ссылка, ВидПочтовогоАдресаПолучателей, Личная, ТипПолучателейРассылки";
	КонецЕсли;
	
	ПараметрыПолучателей = Новый Структура(НаборПараметров);
	ЗаполнитьЗначенияСвойств(ПараметрыПолучателей, Объект);
	
	Если Объект.Личная = ЛОЖЬ Тогда
		МассивПолучатели = Новый Массив;
		Для Каждого ТекСтрока Из Объект.Получатели Цикл
			Если НЕ ТекСтрока.Исключен Тогда
				МассивПолучатели.Добавить(ТекСтрока.Получатель);
			КонецЕсли;
		КонецЦикла;
		ПараметрыПолучателей.Вставить("Получатели", МассивПолучатели);
	КонецЕсли;
