...
// Функция возвращает значение настройки для переданного пользователя без учета шаблона прав
//
// Параметры:
//	Пользователь	- СправочникСсылка				- Пользователь, для которого получается настройка
//	Настройки		- ПланВидовХарактеристикСсылка	- Настройка из ПВХ НастройкиПользователей
//
// Возвращаемое значение:
//	Произвольный	- Значение настройки.
//
Функция сфпПолучитьИндивидуальнуюНастройкуПользователя(Пользователь, Настройка)
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	ПВХ_ПраваИНастройки.ТипЗначения КАК ТипЗначения,
	|	ЕСТЬNULL(ПраваИНастройки.Значение, ПВХ_ПраваИНастройки.ЗначениеПоУмолчанию) КАК Значение
	|ИЗ
	|	ПланВидовХарактеристик.ПраваИНастройки КАК ПВХ_ПраваИНастройки
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПраваИНастройки КАК ПраваИНастройки
	|	ПО (ПВХ_ПраваИНастройки.Ссылка = ПраваИНастройки.ПравоНастройка)
	|	И ((ПВХ_ПраваИНастройки.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначениеПравИНастроек.Пользователь) И ПраваИНастройки.Объект = &Пользователь) ИЛИ
	|	   (ПВХ_ПраваИНастройки.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначениеПравИНастроек.Подразделение) И ПраваИНастройки.Объект = &Подразделение) ИЛИ
	|	   (ПВХ_ПраваИНастройки.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначениеПравИНастроек.Организация) И ПраваИНастройки.Объект = &Организация) ИЛИ
	|	   (ПВХ_ПраваИНастройки.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначениеПравИНастроек.Компания) И ПраваИНастройки.Объект = НЕОПРЕДЕЛЕНО))
	|ГДЕ 
	|	НЕ ПВХ_ПраваИНастройки.ЭтоГруппа
	|	И НЕ ПВХ_ПраваИНастройки.ПометкаУдаления
	|	И ПВХ_ПраваИНастройки.Ссылка = &Право");
	
	Если ТипЗнч(Настройка) <> Тип("ПланВидовХарактеристикСсылка.ПраваИНастройки") Тогда
		Настройка = обПолучитьСсылкуПВХПравИНастроек(Настройка);
	КонецЕсли;
	Запрос.УстановитьПараметр("Право", Настройка);
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	Запрос.УстановитьПараметр("Подразделение", Пользователь.Подразделение);
...
КонецФункции // сфпПолучитьНастройкуПользователя()
// -АА
...
Функция сфпПолучитьЗначениеНастройкиПользователя(ИмяНастройки, Пользователь = Неопределено) Экспорт
	...
	Если ТипЗнч(ИмяНастройки) = Тип("ПланВидовХарактеристикСсылка." + ИмяПВХ_СофтфонНастройкиПользователей) Тогда
		  НастройкаПВХ = ИмяНастройки;
	Иначе НастройкаПВХ = ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей][ИмяНастройки];
	КонецЕсли;

	// +АА
	Если НастройкаПВХ = ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей].сфпТекущийВнутреннийНомер Тогда
		Возврат сфпПолучитьИндивидуальнуюНастройкуПользователя(Пользователь, НастройкаПВХ);
	КонецЕсли;
	
	Возврат обПолучитьПраваИНастройкиПользователя(Пользователь, НастройкаПВХ);
	// -АА
...
Функция сфпПараметрыСервера() Экспорт 
	...
	Структура.Вставить("ПрефиксВыходаВМежгород",				Константы.CRM_ПрефиксВыходаНаМежгород.Получить());
	Структура.Вставить("ПрефиксВыходаНаМеждународную",			Константы.CRM_ПрефиксВыходаНаМеждународную.Получить());
	Структура.Вставить("МаксимальнаяДлинаВнутреннихНомеров",	Константы.CRM_МаксимальнаяДлинаВнутреннегоНомера.Получить());
	Структура.Вставить("ПоследниеЦифрыТелефонногоНомера",		Константы.CRM_КоличествоХранимыхЦифрТелефона.Получить());
	Структура.Вставить("КодСтраны",								Константы.CRM_КодСтраны.Получить());
	Структура.Вставить("КодГорода",								Константы.CRM_КодГорода.Получить());
	Структура.Вставить("ПрефиксВыходаВГород",					Константы.CRM_ПрефиксВыходаВГород.Получить());
	// -АА
...
//
Процедура сфпЗаписатьПараметрыСервера(СтруктураНастроек) Экспорт
	...
	Если СтруктураНастроек.Свойство("PrefCity") И НЕ (СтруктураНастроек["PrefCity"]  = Неопределено) Тогда
		...
		Константы.CRM_ПрефиксВыходаВГород.Установить(СтруктураНастроек["PrefCity"]);
		// -АА
	КонецЕсли;		
	Если СтруктураНастроек.Свойство("PrefContry") И НЕ (СтруктураНастроек["PrefContry"] = Неопределено) Тогда
		...
		Константы.CRM_ПрефиксВыходаНаМежгород.Установить(СтруктураНастроек["PrefContry"]);
		// -АА
	КонецЕсли;		
	Если СтруктураНастроек.Свойство("PrefWorld") И НЕ (СтруктураНастроек["PrefWorld"] = Неопределено) Тогда
		...
		Константы.CRM_ПрефиксВыходаНаМеждународную.Установить(СтруктураНастроек["PrefWorld"]);
		// -АА
	КонецЕсли;		
	Если СтруктураНастроек.Свойство("InternalNumMaxLen") И НЕ (СтруктураНастроек["InternalNumMaxLen"] = Неопределено) Тогда
		...
		Константы.CRM_МаксимальнаяДлинаВнутреннегоНомера.Установить(СтруктураНастроек["InternalNumMaxLen"]);
		// -АА
	КонецЕсли;		
	...
	Если СтруктураНастроек.Свойство("LastNumberCount") И НЕ (СтруктураНастроек["LastNumberCount"] = Неопределено) Тогда
		...
		Константы.CRM_КоличествоХранимыхЦифрТелефона.Установить(СтруктураНастроек["LastNumberCount"]);
		// -АА
	КонецЕсли;		
	...
	Если СтруктураНастроек.Свойство("ContryCode") И НЕ (СтруктураНастроек["ContryCode"] = Неопределено) Тогда
		...
		Константы.CRM_КодСтраны.Установить(СтруктураНастроек["ContryCode"]);	
		// -АА
	КонецЕсли;	
	Если СтруктураНастроек.Свойство("CityCode") И НЕ (СтруктураНастроек["CityCode"] = Неопределено) Тогда
		...
		Константы.CRM_КодГорода.Установить(СтруктураНастроек["CityCode"]);
		// -АА
	КонецЕсли;		
	Если СтруктураНастроек.Свойство("WSDL") И НЕ (СтруктураНастроек["WSDL"] = Неопределено) Тогда
		...
		Константы.CRM_ИмяСервера.Установить(сфпИмяСервера);	
		//Константы.сфпПорт.Установить(сфпПорт);	
		Константы.CRM_Порт.Установить(сфпПорт);
		// -АА
	КонецЕсли;	
...
Функция сфпУбратьИзНомераТелефонаВсеПрефиксы(НомерТелефона) Экспорт
	...
	Если Лев(ОчищенныйНомер, 3) = "000" И СтрДлина(ОчищенныйНомер) <= МаксимальнаяДлинаВнутреннихНомеров + 3 И СтрДлина(ОчищенныйНомер) < 7 Тогда
		ОчищенныйНомер = Сред(ОчищенныйНомер, 4);

	ИначеЕсли Лев(ОчищенныйНомер, 2) = "00" И СтрДлина(ОчищенныйНомер) <= МаксимальнаяДлинаВнутреннихНомеров + 2 И СтрДлина(ОчищенныйНомер) < 7 Тогда
		ОчищенныйНомер = Сред(ОчищенныйНомер, 3);

	ИначеЕсли Лев(ОчищенныйНомер, 1) = "0" И СтрДлина(ОчищенныйНомер) <= МаксимальнаяДлинаВнутреннихНомеров + 1 И СтрДлина(ОчищенныйНомер) < 7 Тогда
...
Функция сфпУбратьИзНомераТелефонаВсеПрефиксыВыходаНаЛинию(НомерТелефона) Экспорт
	...
	Если СтрДлина(НомерТелефона) > 10 Тогда
		...
		ПрефиксВыхода = Константы.CRM_ПрефиксВыходаВГород.Получить();
		// -АА
		...
		Если СтрДлина(НомерТелефона) > 10 Тогда
			...
			ПрефиксВыхода = Константы.CRM_ПрефиксВыходаНаМежгород.Получить();
			// -АА
			Если Найти(НомерТелефона, ПрефиксВыхода) = 1 Тогда
				НомерТелефона = Сред(НомерТелефона, СтрДлина(ПрефиксВыхода) + 1);
			КонецЕсли;	
		...
		Если СтрДлина(НомерТелефона) > 10 Тогда
			...
			ПрефиксВыхода = Константы.CRM_ПрефиксВыходаНаМеждународную.Получить();
			// -АА
			Если Найти(НомерТелефона, ПрефиксВыхода) = 1 Тогда
				НомерТелефона = Сред(НомерТелефона, СтрДлина(ПрефиксВыхода) + 1);
			КонецЕсли;	
...
Процедура сфпЗаполнитьРегистрПоискаПоНомерам(Объект, ПроверкаНаКИ = Истина, ЗаписьМаршрутизации = Ложь) Экспорт
	...
	// +АА
	Если Метаданные.РегистрыСведений.Найти("сфпКонтактыТелефонии") <> Неопределено Тогда
		Если НаборЗаписан И ТекМетаданные.Имя <> "Партнеры" И ТекМетаданные.Имя <> "Контрагенты" И ТекМетаданные.Имя <> "Организации" Тогда
			ТелефонКонтакта = "";
...
Процедура сфпЗаполнитьРегистрПоискаНомеровПоТипуОбъекта(ТипОбъекта) Экспорт
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		Иначе	
			сфпПользователь	= НаборЗаписей[0].Пользователь;
			сфпВнутреннийНомер = сфпТекущийВнутреннийНомер(НаборЗаписей[0].Пользователь);
			Если ЕстьПользовательДляПереключенияЗвонков Тогда
				Если сфпПользователь <> ВыборкаОбъектов.ПользовательДляПереключенияЗвонков И
					 ЗначениеЗаполнено(сфпТекущийВнутреннийНомер(ВыборкаОбъектов.ПользовательДляПереключенияЗвонков, Истина)) Тогда
					сфпПользователь = ВыборкаОбъектов.ПользовательДляПереключенияЗвонков;
		...
		// +АА
		Если Метаданные.РегистрыСведений.Найти("сфпКонтактыТелефонии") <> Неопределено Тогда
			Если НаборЗаписан И ТипОбъекта <> "Партнеры" И ТипОбъекта <> "Контрагенты" И ТипОбъекта <> "Организации" Тогда
				ТелефонКонтакта = "";
				ТелефонКонтактаПредставление = "";
...
Функция сфпПолучитьМассивТелефоновИФаксов(Ссылка) Экспорт
	...
	Если Метаданные.Справочники.ВидыКонтактнойИнформации.Реквизиты.Найти("РеквизитДопУпорядочивания") <> Неопределено Тогда
		СтрокаУпорядочивания = "
		|УПОРЯДОЧИТЬ ПО Вид.РеквизитДопУпорядочивания";
	КонецЕсли;
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ Вид, Представление
	|ИЗ РегистрСведений.сфпНомераТелефоновДляПоиска
	|ГДЕ Объект В (&Объекты)" + СтрокаУпорядочивания);
	
	Объекты = Новый Массив();
	Объекты.Добавить(Ссылка);
	Если Ссылка.Метаданные().Имя = "Пользователи" И Метаданные.Справочники.Пользователи.Реквизиты.Найти("ФизЛицо") <> Неопределено Тогда
		Если ЗначениеЗаполнено(Ссылка.ФизЛицо) Тогда
			Объекты.Добавить(Ссылка.ФизЛицо);
		КонецЕсли;
	КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтруктураТелефона = Новый Структура("Вид,Представление", Выборка.Вид, Выборка.Представление);
...
Функция сфпНайтиОбъектВРегистреПоТелефону(НомерТелефона, МассивВладельцевКонтактов = Неопределено) Экспорт
	...
	Если СтрДлина(ОчищенныйНомерТелефона) > 0 Тогда
		...
		ДлинаВнутреннихНомеров = Константы.CRM_МаксимальнаяДлинаВнутреннегоНомера.Получить();
		//КоличествоЦифрВНомере = Константы.сфпПоследниеЦифрыТелефонногоНомера.Получить();
		КоличествоЦифрВНомере = Константы.CRM_КоличествоХранимыхЦифрТелефона.Получить();
		// -АА
		...
		|ГДЕ
		|	НомераТелефонов.НомерТелефона = &НомерТелефона
		|УПОРЯДОЧИТЬ ПО
		|	Порядок";
...
Функция сфпТекущийВнутреннийНомер(Пользователь = Неопределено, НомерДляМаршрутизации = Ложь) Экспорт
	...
	Если НомерДляМаршрутизации И НЕ ЗначениеЗаполнено(сфпВнутреннийНомер) Тогда
...
Функция сфпПолучитьВнутреннийНомерПользователяИзКИ(Пользователь = Неопределено) Экспорт
	...
	ПредопределенноеЗначениеКИ = Справочники.ВидыКонтактнойИнформации.ПустаяСсылка();
	Если НЕ ЗначениеЗаполнено(ПредопределенноеЗначениеКИ) Тогда
		Попытка
			ПредопределенноеЗначениеКИ = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.CRM_ТелефонПользователяРабочий");
		Исключение КонецПопытки;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ПредопределенноеЗначениеКИ) Тогда
		Попытка
			ПредопределенноеЗначениеКИ = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ТелефонРабочий");
		Исключение КонецПопытки;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПредопределенноеЗначениеКИ) Тогда
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      |	КонтактнаяИнформация.Представление КАК НомерТелефона
		                      |ИЗ
		                      |	Справочник.Пользователи КАК Пользователи
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
		                      |		ПО КонтактнаяИнформация.Объект = Пользователи.Сотрудник
		...
		Запрос.УстановитьПараметр("Пользователь", Пользователь);
		Запрос.УстановитьПараметр("ВидКИ", ПредопределенноеЗначениеКИ);
...
Функция сфпЗаписатьНомерПользователю(НомерЛинии, Пользователь) Экспорт
	...
	Если НЕ ПустаяСтрока(НомерЛинии) Тогда
		...
		// +АА
		//ИмяИзмеренияПользователь = "Пользователь";
		//ИмяИзмеренияНастройка = "Настройка";
		ИмяИзмеренияПользователь = "Объект";
		ИмяИзмеренияНастройка = "ПравоНастройка";
		// -АА
		
		НаборЗаписей = РегистрыСведений[ИмяРегистраСофтфонНастройкиПользователей].СоздатьНаборЗаписей();
		НаборЗаписей.Отбор[ИмяИзмеренияНастройка].Установить(ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей].сфпТекущийВнутреннийНомер);
		НаборЗаписей.Прочитать();
		Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
			Если ЗаписьНабора[ИмяИзмеренияПользователь] = Пользователь Тогда
				ЗаписьНабора.Значение = НомерЛинии;
				НомерЗаписан = Истина;
				
			ИначеЕсли ЗаписьНабора.Значение = НомерЛинии Тогда
				ЗаписьНабора.Значение = "";
				МассивПользователей.Добавить(ЗаписьНабора[ИмяИзмеренияПользователь]);	
			КонецЕсли;	
		КонецЦикла;
		
		Если НЕ НомерЗаписан Тогда
			ЗаписьНабора = НаборЗаписей.Добавить();
			ЗаписьНабора[ИмяИзмеренияПользователь] = Пользователь;
			ЗаписьНабора[ИмяИзмеренияНастройка] = ПланыВидовХарактеристик[ИмяПВХ_СофтфонНастройкиПользователей].сфпТекущийВнутреннийНомер;
			ЗаписьНабора.Значение = НомерЛинии;
		КонецЕсли;	
...
Процедура сфпЗаменитьМаршрутизациюВАТС(СписокМаршрутизации) Экспорт
	...
	#Иначе
		...
		АдресСервера	= Константы.CRM_ИмяСервера.Получить();
		//ПортСервера	= Константы.сфпПорт.Получить();
		ПортСервера		= Константы.CRM_Порт.Получить();
...
Функция сфпЗаполненыНастройкиСервера()
	...
	Если ПустаяСтрока(Константы.CRM_ИмяСервера.Получить()) Тогда
	// -АА	
		ЕстьОшибки = Истина;	
	КонецЕсли;	
	// +АА
	//Если ПустаяСтрока(Константы.сфпПорт.Получить()) Тогда
	Если ПустаяСтрока(Константы.CRM_Порт.Получить()) Тогда	
	// -АА
		ЕстьОшибки = Истина;	
	КонецЕсли;
...
Функция сфпПолучитьМассивВсехПользователей(МассивВидов) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ Объект КАК Пользователь, Значение
	|	                  ИЗ РегистрСведений." + ИмяРегистраСофтфонНастройкиПользователей + "
	|	                  ГДЕ ПравоНастройка = &Настройка) КАК Настройки
			...
			Если ЗначениеЗаполнено(ИмяКниги) И ЗначениеЗаполнено(ИдентификаторКниги) Тогда
				...
				Если ЗначениеЗаполнено(ИдентификаторВнутреннегоНомера) ИЛИ ЗначениеЗаполнено(ИдентификаторМобильногоНомера) Тогда
					...
					Для Каждого ТекКонтакт Из КонтактыКниги Цикл
						...
						к = МассивОписанияКонтакта.Количество() - 1;
						Пока к >= 0 Цикл
							ОписаниеКонтакта = ОписаниеКонтакта + ?(ОписаниеКонтакта = "", "", "/") + МассивОписанияКонтакта[к];
...
Функция сфпСоздатьТелефонныйЗвонок(СтруктураЗвонка, ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(НовыйЗвонок) Тогда
		...
		Если НЕ ПустаяСтрока(ИмяРеквизитаАбонентКонтакт) Тогда
			НоваяЗапись.АбонентКонтакт = НовыйЗвонок[ИмяРеквизитаАбонентКонтакт];
		КонецЕсли;
...
//	Нет.
//
Процедура сфпПолучитьИсториюЗвонков(ПараметрыВыполнения = Неопределено) Экспорт
	...
	ДлинаНомера = Константы.CRM_КоличествоХранимыхЦифрТелефона.Получить();
	//ДлинаВнешнегоНомера = Константы.сфпМаксимальнаяДлинаВнутреннихНомеров.Получить();
	ДлинаВнешнегоНомера = Константы.CRM_МаксимальнаяДлинаВнутреннегоНомера.Получить();
	// -АА
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе	
			...
			Если ЗвонокОбъект.Ссылка.Метаданные().Реквизиты.Найти("Важность") <> Неопределено Тогда
				Если НЕ ЗначениеЗаполнено(ЗвонокОбъект["Важность"]) Тогда
					...
					// +АА
					УстановитьЗначениеРеквизита(ЗвонокОбъект, "Важность", Перечисления.Важность.Низкая, ТребуетсяЗаписьЗвонка);
					// -АА
				КонецЕсли;
			КонецЕсли;
			...
			// +АА
			УстановитьЗначениеРеквизита(ЗвонокОбъект, "Организация", ЗвонокОбъект.Ответственный.Организация, ТребуетсяЗаписьЗвонка);
			УстановитьЗначениеРеквизита(ЗвонокОбъект, "ПодразделениеКомпании", ЗвонокОбъект.Ответственный.Подразделение, ТребуетсяЗаписьЗвонка);
			УстановитьЗначениеРеквизита(ЗвонокОбъект, "ХозОперация", ПредопределенноеЗначение("Справочник.ХозОперации.ТелефонныйЗвонок"), ТребуетсяЗаписьЗвонка);
			// -АА
...
Процедура сфпЗаписатьИдентификаторЗаписиВРегистр(Звонок, ИдентификаторЗвонка, ИдентификаторЗаписи) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторЗвонка) ИЛИ НЕ ЗначениеЗаполнено(ИдентификаторЗаписи) Тогда
		Возврат;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МенеджерЗаписи.Выбран() Тогда
			...
			Если МенеджерЗаписи.ИдентификаторЗаписи <> ИдентификаторЗаписи Тогда
				МенеджерЗаписи.ИдентификаторЗаписи = ИдентификаторЗаписи;
				ЗаписатьМенеджер = Истина;
			КонецЕсли;
			Если НЕ ЗначениеЗаполнено(МенеджерЗаписи.АбонентКонтакт) И МенеджерЗаписи.АбонентКонтакт <> Выборка.АбонентКонтакт Тогда
...
Функция сфпНайтиПоследнийЗвонокПоНомеру(НомерТелефона, ДатаЗвонка)
	...
	|УПОРЯДОЧИТЬ ПО Звонок.Дата УБЫВ"); // +АА
	Запрос.УстановитьПараметр("НомерТелефона", НомерТелефона);
...
Функция АА_сфпНайтиОтветственного(ВнутреннийНомер) Экспорт
	...
	|	И КонтактнаяИнформация.CRM_ПолеХраненияНомера = &ВнутреннийНомер
	|   И КонтактнаяИнформация.Объект ССЫЛКА Справочник.Сотрудники";
	...
	УстановитьПривилегированныйРежим(Ложь);
	Возврат Ответственный;
КонецФункции // АА_сфпНайтиОтветственного()	
// -АА
...
Функция сфпНайтиОтветственного(ВнутреннийНомер) Экспорт
	...
	// +АА
	Возврат АА_сфпНайтиОтветственного(ВнутреннийНомер);
	// -АА
	...
	|ИЗ РегистрСведений." + ИмяРегистраСофтфонНастройкиПользователей + "
	|ГДЕ Настройка = ЗНАЧЕНИЕ(ПланВидовХарактеристик." + ИмяПВХ_СофтфонНастройкиПользователей + ".сфпТекущийВнутреннийНомер)
	|	 И Значение = &ВнутреннийНомер");
	Запрос.УстановитьПараметр("ВнутреннийНомер", ВнутреннийНомер);
...
Функция сфпПолучитьПропущенныеЗвонки(НомерТелефона)
	...
	|ИЗ РегистрСведений.сфпИсторияЗвонков
	|ГДЕ Входящий И НомерТелефона = &НомерТелефона И Звонок.сфпСостояниеЗвонка = ЗНАЧЕНИЕ(Перечисление.сфпСостоянияЗвонков.Пропущенный)");
	Запрос.УстановитьПараметр("НомерТелефона", НомерТелефона);
...
Процедура сфпЗавершитьПропущенныеЗвонки(НомерТелефона)
	...
	УдалятьНапоминания = (Метаданные.РегистрыСведений.Найти("НапоминанияПользователя") <> Неопределено);
	...
	Для Каждого ЗвонокМассива Из МассивЗвонков Цикл
		...
		Если УдалятьНапоминания Тогда
			сфпУдалитьНапоминанияОПропущенныхЗвонках(ЗвонокМассива.Ссылка);
		КонецЕсли;
	КонецЦикла;
...
//
Процедура сфпУдалитьНапоминанияОПропущенныхЗвонках(Источник = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяДокументаТелефонныйЗвонок) Тогда
		...
		|ГДЕ
		|	Источник ССЫЛКА " + ИмяДокументаТелефонныйЗвонок + "
		|	И Источник.сфпИдентификаторЗвонка > 0";
	...
	ИначеЕсли ИмяОбъекта = ИмяДокументаТелефонныйЗвонок Тогда
		...
		Если НЕ ПустаяСтрока(ИмяРеквизитаАбонентКонтакт) Тогда
			...
			Если ЗначениеЗаполнено(АбонентКонтакт) Тогда
				...
				Если ЗначениеЗаполнено(ВладелецКонтактаОбъект) Тогда
...
Функция сфпТребуетсяНастройкаТелефонии() Экспорт
	...
	ТребуетсяНастройка = НЕ Константы.сфпИспользоватьСофтФон.Получить();
...
Функция сфпМожноОбновитьТелефонныеЗвонки() Экспорт
	...
	// +АА
	Если Метаданные.Документы.ТелефонныйЗвонок.Реквизиты.Найти("сфпКлиент") = Неопределено Тогда
		Возврат Ложь;
...
Функция сфпПроверитьНаличиеНомераТелефонаУАбонента(Абонент, АбонентКакСвязаться) Экспорт
	...
	Если ТипЗнч(Абонент) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		киПрочитатьКонтактнуюИнформацию(НаборЗаписейКИ, Абонент);
		Для Каждого СтрокаКИ ИЗ НаборЗаписейКИ Цикл
			...
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипЗнч(Абонент) = Тип("СправочникСсылка.ПодразделенияКомпании") Тогда
		НаборЗаписейКИ = РегистрыСведений.КонтактнаяИнформация.СоздатьНаборЗаписей();
		киПрочитатьКонтактнуюИнформацию(НаборЗаписейКИ, Абонент);
		Для Каждого СтрокаКИ ИЗ НаборЗаписейКИ Цикл
			Если СтрокаКИ.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
	...
	ИначеЕсли ТипЗнч(Абонент) = Тип("СправочникСсылка.Сотрудники") Тогда
		...
		Для Каждого СтрокаКИ ИЗ НаборЗаписейКИ Цикл
			...
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипЗнч(Абонент) = Тип("СправочникСсылка.Организации") Тогда
		НаборЗаписейКИ = РегистрыСведений.КонтактнаяИнформация.СоздатьНаборЗаписей();
		киПрочитатьКонтактнуюИнформацию(НаборЗаписейКИ, Абонент);
		Для Каждого СтрокаКИ ИЗ НаборЗаписейКИ Цикл
			Если СтрокаКИ.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
