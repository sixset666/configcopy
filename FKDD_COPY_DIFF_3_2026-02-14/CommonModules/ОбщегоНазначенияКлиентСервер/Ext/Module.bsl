...
Процедура ДобавитьМассивОбъектовКСравнению(Объекты) Экспорт
	ОбщегоНазначенияВызовСервера.ДобавитьМассивОбъектовКСравнению(Объекты);
КонецПроцедуры
...
#Иначе
		Возврат ОбщегоНазначенияПовтИсп.КодОсновногоЯзыка();
#КонецЕсли
КонецФункции
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ПредопределенныеЗначения = ОбщегоНазначенияПовтИсп.СсылкиПоИменамПредопределенных(ПолноеИмяОбъектаМетаданных);
#Иначе
		ПредопределенныеЗначения = ОбщегоНазначенияКлиентПовтИсп.СсылкиПоИменамПредопределенных(
...
Функция ИмяПараметраОтмененныхДлительныхОпераций(Параметры) Экспорт
	...
	Возврат "ОтмененныеДлительныеОперации";
КонецФункции
...
Функция СериализоватьЗапросДляОтладки(ОбъектОтладки)
	...
	Если ОбъектОтладки.МенеджерВременныхТаблиц <> Неопределено Тогда
		СтруктураВременныхТаблиц = ОбщегоНазначенияВызовСервера.СтруктураВременныхТаблицМенеджераВременныхТаблиц(
...
Функция СериализоватьСКДДляОтладки(СКД, НастройкиСКД, ВнешниеНаборыДанных)
	Возврат ОбщегоНазначенияВызовСервера.СериализоватьОбъектСКДДляОтладки(СКД, НастройкиСКД, ВнешниеНаборыДанных);
КонецФункции
...
Функция СериализоватьHTTPЗапросДляОтладки(ЗапросHTTP, СоединениеHTTP)
	...
	СтруктураОбъекта.Вставить("Заголовки", ОбщегоНазначенияКлиентСервер.ПолучитьСтрокуЗаголовковHTTP(
...
Функция СериализоватьОбъектДляОтладкиВСтруктуру(ОбъектОтладки, НастройкиСКДИлиСоединениеHTTP, ВнешниеНаборыДанных)
	ТипВсеСсылки = ОбщегоНазначенияПовтИсп.ОписаниеТипаВсеСсылки();
...
#Если Клиент Тогда
	...
	//	ОбщегоНазначенияКлиент.ОткрытьКонсольОтладки(ТипОбъектаОтладки, ДанныеДляОтладки);

#КонецЕсли
		Возврат Неопределено;
	Иначе
		Возврат ОбщегоНазначенияВызовСервера.ЗаписатьДанныеДляОтладкиВСправочник(ТипОбъектаОтладки,
			СериализованныйОбъект);
	КонецЕсли;
КонецФункции

Функция КлючДанныхОбъектаДанныхОтладкиВХранилищеНастроек() Экспорт
	Возврат "УниверсальныеИнструменты_ДанныеДляОтладки";
КонецФункции

Функция КлючОбъектаВХранилищеНастроек() Экспорт
	Возврат "УниверсальныеИнструменты";
КонецФункции
...
#Если ВебКлиент Тогда
	Возврат ОбщегоНазначенияВызовСервера.мПрочитатьJSON(Значение);
#Иначе
...
#Если ВебКлиент Тогда
	Возврат ОбщегоНазначенияВызовСервера.мЗаписатьJSON(СтруктураДанных);
#Иначе
...
Функция КлючДанныхНастроекВХранилищеНастроек() Экспорт
	Возврат "УниверсальныеИнструменты_Настройки";
КонецФункции
