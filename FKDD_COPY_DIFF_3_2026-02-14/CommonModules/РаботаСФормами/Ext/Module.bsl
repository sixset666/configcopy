...
Функция СоздатьКомандуПоОписанию(Форма, ОписаниеКоманды) Экспорт
	...
	Если ОписаниеКоманды.Картинка<>Неопределено Тогда
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоПортативнаяПоставка()
...
//@skip-warning
	...
	ИначеЕсли РодительКнопки = Форма.КоманднаяПанель Тогда
		Возврат Истина;
	ИначеЕсли ТипЗнч(РодительКнопки) = ОбщегоНазначенияКлиентСервер.ТипУправляемойФормы() Тогда
		Возврат Ложь;
	Иначе
...
//@skip-warning
	...
	ИначеЕсли ТипЗнч(РодительЭлемента) = ОбщегоНазначенияКлиентСервер.ТипУправляемойФормы() Тогда
		Возврат "";
	Иначе
...
//			Продолжить;
	...
	МассивДляСозданияЭлементов=ОбщегоНазначенияКлиентСервер.ПараметрыЗаписиДляВыводаНаФормуИнструмента();
	...
	Для Каждого ИмяСоздаваемогоЭлемента Из МассивДляСозданияЭлементов Цикл
		...
		РаботаСФормами.СоздатьЭлементПоОписанию(Форма, ОписаниеЭлемента);
	КонецЦикла;
	...
	РаботаСФормами.СоздатьКомандуПоОписанию(Форма, ОписаниеКнопки);
	РаботаСФормами.СоздатьКнопкуПоОписанию(Форма, ОписаниеКнопки);
КонецПроцедуры
