...
Функция ВыполнитьЗапросПроверкиРНПТ(ПараметрыКоманды) Экспорт
	
	// Переопределим параметры запроса
	ОбменСФНСПереопределяемый.ЗаполнитьПараметрыПроверкиРНПТ(ПараметрыКоманды);
	...
	Организация = ?(ПараметрыКоманды.Свойство("Организация"), ПараметрыКоманды.Организация, Неопределено);
	РНПТ = ?(ПараметрыКоманды.Свойство("РНПТ"), ПараметрыКоманды.РНПТ, Неопределено);
	Отказ = Ложь;
	
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		Результат.ТекстОшибки = "Не указана организация, на основании которой производить проверку";
		Отказ = Истина;
	Иначе
		ПараметрыПодключения = ДанныеПодлюченияКСервису(Организация);
		Если ПараметрыПодключения = Неопределено Тогда
			Результат.ТекстОшибки = "Не указаны настройки подключения к сервису ФНС";
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(РНПТ) Тогда
		Результат.ТекстОшибки = "Не указан РНПТ для проверки.";
		Отказ = Истина;
	КонецЕсли;
...
Функция ДанныеПодлюченияКСервису(Организация)
	...
	МастерТокенФНС = обПолучитьПраваИНастройкиПользователя(Организация, "МастерТокенФНС");
	ВремяОжиданияФНС = обПолучитьПраваИНастройкиПользователя(Организация, "ВремяОжиданияФНС");
	АдресСервераФНС = обПолучитьПраваИНастройкиПользователя(Организация, "АдресСервераФНС");
	
	// Получим адрес и порт отдельно
	МассивАдреса = СтрРазделить(АдресСервераФНС, ":", Ложь);
	Попытка
		Порт = Число(МассивАдреса[МассивАдреса.Количество() - 1]);
		АдресСервера = СтрЗаменить(АдресСервераФНС, ":" + Строка(Порт), "");
	Исключение
		Возврат Неопределено;
	КонецПопытки;
	
	Если НЕ ЗначениеЗаполнено(АдресСервера) ИЛИ Порт = 0
		ИЛИ НЕ ЗначениеЗаполнено(МастерТокенФНС) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Подключение = Новый Структура();
	Подключение.Вставить("МастерТокен", МастерТокенФНС);
	Подключение.Вставить("ВремяОжидания", ВремяОжиданияФНС);
	Подключение.Вставить("АдресСервера", АдресСервера);
	Подключение.Вставить("Порт", Порт);
...
Процедура ОтправитьЗапросАвторизации(ПараметрыПодключения, Результат)
	...
	Если ДанныеОтвета.Свойства().Получить("Body") <> Неопределено Тогда
		...
		Если ДанныеОтвета.Body.Свойства().Получить("GetMessageResponse") <> Неопределено Тогда
			Если ДанныеОтвета.Body.GetMessageResponse.Message.AuthResponse.Свойства().Получить("Fault") <> Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					"Ошибка выполнения команды авторизации в ФНС",
				УровеньЖурналаРегистрации.Ошибка, , ,
				СтрШаблон("Ошибка при авторизации на сервисе: %1",
					ДанныеОтвета.Body.GetMessageResponse.Message.AuthResponse.Fault.Message));
				Результат.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
		...
		ИначеЕсли ДанныеОтвета.Body.Свойства().Получить("Fault") <> Неопределено Тогда
			ЗаписьЖурналаРегистрации(
				"Ошибка выполнения команды авторизации в ФНС",
			УровеньЖурналаРегистрации.Ошибка, , ,
			СтрШаблон("Ошибка при авторизации на сервисе: %1", ДанныеОтвета.Body.Fault.faultstring));
			Результат.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(
		"Ошибка выполнения команды авторизации в ФНС",
		УровеньЖурналаРегистрации.Ошибка, , ,
		"Ошибка при выполнении авторизации на сервисе.");
	Результат.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
...
Процедура ОтправитьЗапросПроверкиРНПТ(АдресСервера, Порт, ВремяОжидания, МастерТокен, РНПТ, Результат)
	...
	Если ДанныеОтвета.Свойства().Получить("Body") <> Неопределено Тогда
		...
		Если ДанныеОтвета.Body.Свойства().Получить("SendMessageResponse") <> Неопределено Тогда
			Если ДанныеОтвета.Body.SendMessageResponse.Свойства().Получить("Fault") <> Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					"Ошибка выполнения команды проверки РНПТ в ФНС",
				УровеньЖурналаРегистрации.Ошибка, , ,
				СтрШаблон("Ошибка при проверке РНПТ в ФНС на сервисе: %1",
		...
		ИначеЕсли ДанныеОтвета.Body.Свойства().Получить("Fault") <> Неопределено Тогда
			ЗаписьЖурналаРегистрации(
				"Ошибка выполнения команды проверки РНПТ в ФНС",
			УровеньЖурналаРегистрации.Ошибка, , ,
			СтрШаблон("Ошибка при проверке РНПТ в ФНС на сервисе: %1", ДанныеОтвета.Body.Fault.faultstring));
			Результат.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(
		"Ошибка выполнения команды проверки РНПТ в ФНС",
		УровеньЖурналаРегистрации.Ошибка, , ,
		"Ошибка при выполнении проверки РНПТ в ФНС.");
	Результат.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
		...
		Если ДанныеОтвета.Body.Свойства().Получить("GetMessageResponse") <> Неопределено Тогда
			Если ДанныеОтвета.Body.GetMessageResponse.Свойства().Получить("Fault") <> Неопределено Тогда
				ЗаписьЖурналаРегистрации(
					"Ошибка выполнения команды проверки РНПТ в ФНС",
				УровеньЖурналаРегистрации.Ошибка, , ,
				СтрШаблон("Ошибка при получении результата проверки РНПТ в ФНС на сервисе: %1",
...
Функция ОтправитьЗапрос(АдресСервераAPI, Порт, ВремяОжидания, АдресСервиса, ЗаголовокHTTP, ТекстЗапроса)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			"Ошибка выполнения команды сервиса обмена с ФНС",
			УровеньЖурналаРегистрации.Ошибка, , ,
			СтрШаблон("Ошибка получении данных при обращении к %1"
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), АдресСервиса));
		РезультатОтвета.ТекстОшибки = "Ошибка выполнения команды сервиса. См. журнал регистрации.";
...
Процедура СформироватьРезультатПроверкиРНПТ(Сообщение, Результат)
	...
	ИначеЕсли РезультатПроверки.STATUS_CODE = "2" Тогда
		...
		ПредставлениеПроверки.Добавить(СтрШаблон("Код ТН ВЭД: %1", ИнформацияОРНПТ.CODE_TNVED));
		ПредставлениеПроверки.Добавить(СтрШаблон("Товар: %1", ИнформацияОРНПТ.NAME_GOODS));
		ПредставлениеПроверки.Добавить(СтрШаблон(
			"Страна: [%1] %2",
			ИнформацияОРНПТ.CODE_CO_PROD,
			ИнформацияОРНПТ.NAME_CO_PROD));
		ПредставлениеПроверки.Добавить(СтрШаблон(
			"Дата выпуска/регистрации товара: %1",
		...
		ПредставлениеПроверки.Добавить(СтрШаблон(
			"Количество ввезенного товара: %1",
			ИнформацияОРНПТ.QUANT_INPUT_GOODS));
		ПредставлениеПроверки.Добавить(СтрШаблон(
			"Количество выбывшего товара: %1",
			ИнформацияОРНПТ.QUANT_OUTPUT_GOODS));
		ПредставлениеПроверки.Добавить(СтрШаблон(
			"Количество товара в обороте: %1",
