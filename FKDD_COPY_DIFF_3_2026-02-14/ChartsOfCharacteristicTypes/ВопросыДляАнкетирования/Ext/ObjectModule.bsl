// Модуль ПВХ Вопросы для анкетирования
...
//
Функция ПолучитьИзображениеОбъекта() Экспорт 
	Результат = Новый Картинка;
	ВыборкаКартинокОбъекта = РегистрыСведений.КартинкиИФайлы.Выбрать(Новый Структура("Объект", ЭтотОбъект.Ссылка));
	Если ВыборкаКартинокОбъекта.Следующий() Тогда
		Если ВыборкаКартинокОбъекта.Картинка Тогда
			Если ВыборкаКартинокОбъекта.ДанныеВоВнешнемФайле Тогда
				ФайлКартинки = Новый Файл(ВыборкаКартинокОбъекта.ИмяФайла);
				Если ФайлКартинки.Существует() Тогда
					Результат = Новый Картинка(ВыборкаКартинокОбъекта.ИмяФайла, Ложь);
				КонецЕсли;
			Иначе
				ДанныеКартинки = ВыборкаКартинокОбъекта.Данные.Получить();
				Если ТипЗнч(ДанныеКартинки) = Тип("Картинка") Тогда
					Результат = Новый Картинка();
					Результат = ДанныеКартинки;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	Если НЕ обПраво("РедактированиеВопросовАнкет",Права)Тогда
		Отказ=Истина;
		Сообщить("Вопросы для анкетирования изменять запрещено!",СтатусСообщения.Важное); 
	КонецЕсли;
