...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	ЕстьОтборПоНоменклатуре = (ОтчетыСервер.КомпоновщикСчетчикУсловийПоля(Новый ПолеКомпоновкиДанных("Номенклатура"), ОтборКомпоновки)>0);
	ЕстьОтборПоРаботе       = (ОтчетыСервер.КомпоновщикСчетчикУсловийПоля(Новый ПолеКомпоновкиДанных("Работа"),       ОтборКомпоновки)>0);
	...
	Если ЕстьОтборПоНоменклатуре Тогда
		...
		СоответствиеЗамен.Вставить("ВидОплаты",             "Ссылка.ВидОплаты");
		...
		ПоляОграничения.Добавить("ВидОплаты");
		ПоляОграничения.Добавить("ЗаказНаряд");
		ПоляОграничения.Добавить("Номенклатура");
		
		ТекстОтбора = ОтчетыСервер.КомпоновщикПолучитьТекстОтбора(ОтборКомпоновки.Элементы, ОтборКомпоновки.ДоступныеПоляОтбора, "ДокументЗаказНаряд", Запрос.Параметры, СоответствиеЗамен, ПоляОграничения);
	...
	ИначеЕсли ЕстьОтборПоРаботе Тогда
		...
		ПоляОграничения.Добавить("ВидОплаты");
		ПоляОграничения.Добавить("ЗаказНаряд");
		ПоляОграничения.Добавить("Работа");
		...
		|	ТаблицаЗаказНарядов
		|ИЗ
		|	Документ.ЗаказНаряд.Работы КАК ДокументЗаказНаряд
		|ГДЕ
		|	ДокументЗаказНаряд.Ссылка.ПометкаУдаления = ЛОЖЬ И 
		|	((ДокументЗаказНаряд.Ссылка.ДатаСоздания < &ДатаНач И 
		|		 (ДокументЗаказНаряд.Ссылка.ДатаОкончания = &ПустаяДата ИЛИ ДокументЗаказНаряд.Ссылка.ДатаОкончания >= &ДатаНач ИЛИ ДокументЗаказНаряд.Ссылка.ДатаЗакрытия >= &ДатаНач)) ИЛИ 
