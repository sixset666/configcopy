...
Функция ПолучитьТаблицуИсточникаДанных(ТекПостроитель, ТаблицаВывода, ПоляНастройкиОтчета) Экспорт
	...
	Для Каждого ЭлементОтбора Из ПостроительОтчета.Отбор Цикл
		...
		Иначе
			...
			СтрокаУсловияДвижений=СтрокаУсловияДвижений+" И "+Символы.ПС+СтрЗаменить(ТекстУсловие,"#", "ТоварыВПроизводстве."+ПутьКДанным);
		КонецЕсли;
	...
	|			ИЗ
	|				РегистрНакопления.ТоварыВПроизводстве.Остатки(&ДатаНач,"+СтрокаУсловия+") КАК ТоварыВПроизводствеОстатки
	|			
	|			ОБЪЕДИНИТЬ ВСЕ
	|			
	|			ВЫБРАТЬ
	...
	|			ГДЕ
	|				ТоварыВПроизводстве.Период МЕЖДУ &ДатаНач И &ДатаКон
...
Функция СформироватьДиаграмму(ПолеДиаграммы=Неопределено, СтруктураПараметров=Неопределено) Экспорт
	...
	Если ПолеДиаграммы.ТипДиаграммы = ТипДиаграммы.Круговая ИЛИ ПолеДиаграммы.ТипДиаграммы = ТипДиаграммы.КруговаяОбъемная Тогда
		Предупреждение("Отчет динамика остатков незавершенного производства не может использоваться с типом диаграммы """+СокрЛП(ПолеДиаграммы.ТипДиаграммы)+"""");
