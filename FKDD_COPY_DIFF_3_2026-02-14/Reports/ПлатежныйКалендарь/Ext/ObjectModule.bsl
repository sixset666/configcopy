...
Процедура СкопироватьГруппировки(НоваяСтруктураОтчета, Группировка, ТипГруппировкиСтруктуры = Неопределено)
	...
	Если Группировка.Структура.Количество()>0 И 
		(ТипЗнч(Группировка.Структура[0]) = Тип("ГруппировкаКомпоновкиДанных") ИЛИ 
		ТипЗнч(Группировка.Структура[0]) = Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
...
Процедура СформироватьНастройкиПлатежногоКалендаря(НастройкиКалендаря, НастройкиПользователя, ПоВидуДенежныхСредств)
	...
	ОсновнаяГруппировка = ОтчетыСервер.СКД_ДобавитьГруппировку(ТаблицаОтчета.Строки, Новый ПолеКомпоновкиДанных("ВидИтогов"));
	ГруппировкаПериод   = ОтчетыСервер.СКД_ДобавитьГруппировку(ТаблицаОтчета.Колонки, Новый ПолеКомпоновкиДанных("Период"));
	...
	Если НастройкиПользователя.Структура.Количество()>0 Тогда
		...
		Если ТипЗнч(НастройкиПользователя.Структура[0]) = Тип("ТаблицаКомпоновкиДанных") Тогда
			Если НастройкиПользователя.Структура[0].Строки.Количество()>0 Тогда
				//// Сбросим настройки пользовательских настроек
				//СкопироватьГруппировки(НастройкиПользователя.Структура, НастройкиПользователя.Структура[0].Строки[0], Тип("ГруппировкаКомпоновкиДанных"));
				СкопироватьГруппировки(ОсновнаяГруппировка.Структура, НастройкиПользователя.Структура[0].Строки[0]);
			КонецЕсли;
			//НастройкиПользователя.Структура.Удалить(НастройкиПользователя.Структура[0]);
		Иначе
	...
	Если ПоВидуДенежныхСредств Тогда
		ОсновнаяГруппировка = ОтчетыСервер.СКД_ДобавитьГруппировку(ГруппировкаПериод.Структура, Новый ПолеКомпоновкиДанных("ВидДенежныхСредств"));
...
Функция ПолучениеОграниченийНастроек() Экспорт
	...
	СтруктураПолей.Вставить("Колонки");
	СтруктураПолей.Вставить("РежимВыводаОтчета");
	СтруктураПолей.Вставить("Порядок");
	СтруктураПолей.Вставить("ДопГруппировки");
	
	Возврат СтруктураПолей;
	
КонецФункции

Функция ПолучитьДоступныеДействияРасшифровки() Экспорт
	
	ДоступныеДействияРасшифровки = Новый Массив;
	ДоступныеДействияРасшифровки.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.ОткрытьЗначение);
	ДоступныеДействияРасшифровки.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать);
	ДоступныеДействияРасшифровки.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Упорядочить);
	ДоступныеДействияРасшифровки.Добавить(ДействиеОбработкиРасшифровкиКомпоновкиДанных.Оформить);
	
	Возврат ДоступныеДействияРасшифровки;
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	Периодичность         = ОтчетыСервер.КомпоновщикПолучитьЗначениеПараметра("Периодичность",          НастройкиОтчета);
	...
	ВалютаОтчета          = ОтчетыСервер.КомпоновщикПолучитьЗначениеПараметра("ВалютаОтчета",           НастройкиОтчета);
	ПоВидуДенежныхСредств = ОтчетыСервер.КомпоновщикПолучитьЗначениеПараметра("ПоВидуДенежныхСредств",  НастройкиОтчета);
	...
	//КомпоновщикНастроекКалендаря.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ВалютаУпр",     ВалютаУпр);
	...
	//	ОграничениеПериода = Выборка.Период;
	//Иначе
	//	ОграничениеПериода = '00010101';
	//КонецЕсли;
	...
	|	РегистрСведений.КурсыВалют КАК КурсыВалют
	|ГДЕ
	|	КурсыВалют.Период <= &ДатаНачала И 
	...
	|	РегистрСведений.КурсыВалют КАК КурсыВалют
	|ПО
	|	СрезПоследних.Период = КурсыВалют.Период И 
	|	КурсыВалют.Валюта    = &ВалютаОтчета";
	...
	КомпоновщикНастроекКалендаря.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ДатаПрошлогоПериода", ДатаНачала - 1);
	...
	МакетКомпоновки = КомпоновщикМакета.Выполнить(
		КопияСхемы,
		КомпоновщикНастроекКалендаря.Настройки,
		ДанныеРасшифровки
	);
