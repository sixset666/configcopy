Функция Сформировать() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	верс_ВерсииОбъектов.НомерВерсии КАК НомерВерсии,
	                      |	верс_ВерсииОбъектов.Объект КАК Объект
	                      |ИЗ
	                      |	РегистрСведений.верс_ВерсииОбъектов КАК верс_ВерсииОбъектов
	                      |ГДЕ
	                      |	верс_ВерсииОбъектов.ДатаВерсии МЕЖДУ &Дата1 И &Дата2
	                      |	И верс_ВерсииОбъектов.Объект ССЫЛКА Справочник.Контрагенты
	                      |	И верс_ВерсииОбъектов.НомерВерсии <> 1");
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "Контрагенты", ВидСправочника);
	
	ТЗДанных = Новый ТаблицаЗначений;
	ТЗДанных.Колонки.Добавить("Объект");
	ТЗДанных.Колонки.Добавить("Поле");
	ТЗДанных.Колонки.Добавить("СтароеЗначение");  
	ТЗДанных.Колонки.Добавить("НовоеЗначение");  
	ТЗДанных.Колонки.Добавить("Пользователь");
	ТЗДанных.Колонки.Добавить("ДатаИзменения");
	...
	Пока Выборка.Следующий() Цикл
		...
		МассивВерсий.Добавить(Выборка.НомерВерсии-1);
		МассивВерсий.Добавить(Выборка.НомерВерсии);
	
		СформироватьОтчетПоИзменениям(Выборка.Объект, ТЗДанных, МассивВерсий);	
...
Функция СчитатьНачальныеЗначенияРеквизитовИТабличныхЧастей(СсылкаНаОбъект, ТаблицаРеквизитов,
                                                           ТаблицаТЧ,
                                                           КоличествоВерсий,
                                                           МассивНомеровВерсий)
...
Процедура СформироватьОтчетПоИзменениям(СсылкаНаОбъект, ТЗДанных, знач МассивВерсий) Экспорт
	...
	Для ИндексВерсии = 2 По МассивНомеровВерсий.Количество() Цикл
		...
		//Для Каждого СтрокаИзм Из ИзмРек Цикл
		//	
		//	НоваяСтрока = ТЗДанных.Добавить();
		//	НоваяСтрока.Пользователь = Справочники.Пользователи.НайтиПоНаименованию(ВерсияОбъекта.АвторИзменения);
		//	НоваяСтрока.ДатаИзменения = ВерсияОбъекта.ДатаИзменения;	
		//	
		//КонецЦикла;	
	...
	Для Каждого ЭлементМодРеквизит Из ТаблицаИзмененийРеквизитов Цикл
		...
		Если Не ЭлементМодРеквизит._Модификация Тогда
			Продолжить;
		КонецЕсли;	
		...
		Пока ИндексПоВерсиямРеквизитов >= 1 Цикл
			...
			Если  Изменение = Неопределено И НовоеЗначение <> Неопределено Тогда
				
				СтароеЗначение = ПредставлениеЗначенияРеквизита;
				
			ИначеЕсли Изменение = "И" Тогда
				Если ИндексПоВерсиямРеквизитов = 2 Тогда
					НовоеЗначение = ПредставлениеЗначенияРеквизита;
				Иначе
					СтароеЗначение = ПредставлениеЗначенияРеквизита;
				КонецЕсли;	
			...
			Если СтароеЗначение <> Неопределено И НовоеЗначение <> Неопределено И ИндексПоВерсиямРеквизитов = 0 И СокрЛП(СтароеЗначение) <> СокрЛП(НовоеЗначение) Тогда
				...
				НоваяСтрока.Поле = Строка(ОписаниеРеквизита);
	...
	Для Каждого ЭлементИзмененнаяТЧ Из ТаблицаИзмененийТабличныхЧастей Цикл
		...
		Если ИмяТабличнойЧасти <> "БТ_КонтактнаяИнформация"	Тогда
		...
		//	 
		     ...
		     СтараяВерсияТЧ = Неопределено;
			 НоваяВерсияТЧ = Неопределено;
		...
		//		// табличная часть текущей версии (таблица значений с признаками модификации)
				...
				НайденнаяСтрока = ТекущаяТЧВерсии.Найти(ТекСчетчикУникальныйИд, "_ИдСтроки");
		//		
				Если ИндексПоВерсиям = 2 Тогда
			    	НоваяВерсияТЧ = ТекущаяТЧВерсии.Скопировать();
				Иначе
					СтараяВерсияТЧ = ТекущаяТЧВерсии.Скопировать();
				КонецЕсли;	
		...
		//			
					...
					КонецЕсли;
		//			
					Если Модификация = "Д" Тогда
						НоваяСтрока = ТЗДанных.Добавить();
						НоваяСтрока.Объект = СсылкаНаОбъект;
						НоваяСтрока.Пользователь = Справочники.Пользователи.НайтиПоНаименованию(ВерсияОбъекта.АвторИзменения);
						НоваяСтрока.ДатаИзменения = ВерсияОбъекта.ДатаИзменения;	
						НоваяСтрока.НовоеЗначение = НайденнаяСтрока.Представление;
						НоваяСтрока.СтароеЗначение = "";
						НоваяСтрока.Поле = Строка(НайденнаяСтрока.Вид);
					ИначеЕсли Модификация = "У" Тогда
						
					КонецЕсли;	
		
					
		
		//			Секция = СформироватьСекторСтрокиТЧ(МассивЗаполнения, ТипВывода, ТЗДанных, СсылкаНаОбъект, ВерсияОбъекта);
		//			
		//			ТЧОтчета.Присоединить(Секция);
		//			
			...
			Для Каждого Строка Из НоваяВерсияТЧ Цикл
				Если Строка._Модификация = "У" Или Строка._Модификация = "И" Тогда
					
					СтараяСтрока = СтараяВерсияТЧ.Найти(Строка._ИдСтроки, "_ИдСтроки");
					Если СтараяСтрока <> Неопределено Тогда
						...
						НоваяСтрока.Пользователь = Справочники.Пользователи.НайтиПоНаименованию(ВерсияОбъекта.АвторИзменения);
						НоваяСтрока.ДатаИзменения = ВерсияОбъекта.ДатаИзменения;	
						НоваяСтрока.НовоеЗначение = Строка.Представление;
						НоваяСтрока.СтароеЗначение = СтараяСтрока.Представление;
						НоваяСтрока.Поле = Строка(Строка.Вид);
						
						Если ПустаяСтрока(НоваяСтрока.Поле) Тогда
							НоваяСтрока.Поле = Строка(СтараяСтрока.Вид);
						КонецЕсли;	
...
Функция СформироватьСекторСтрокиТЧ(знач ЗначениеЗаполнения,знач ТипВывода = "", ТЗДанных, СсылкаНаОбъект, ВерсияОбъекта)
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПараметров Из Настройки.ПараметрыДанных.Элементы Цикл
		Если СокрЛП(СтрокаПараметров.Параметр) = "ИмяСправочника" Тогда
			СтрокаПараметров.Использование = Истина;
			СтрокаПараметров.Значение = ВидСправочника;
		ИначеЕсли СокрЛП(СтрокаПараметров.Параметр) = "Пользователь" Тогда
			СтрокаПараметров.Использование = Истина;
			СтрокаПараметров.Значение = ПараметрыСеанса.Пользователь;	
		ИначеЕсли СокрЛП(СтрокаПараметров.Параметр) = "ТекДата" Тогда
			СтрокаПараметров.Использование = Истина;
			СтрокаПараметров.Значение = ТекущаяДата();		
		КонецЕсли;			
	КонецЦикла;	
	...
	ВнешнийНаборДанных = Новый Структура("ТЗ", ТЗДанных); 
