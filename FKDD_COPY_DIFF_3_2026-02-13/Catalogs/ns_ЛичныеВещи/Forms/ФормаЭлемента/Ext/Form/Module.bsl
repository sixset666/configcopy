...
Процедура УстановитьПроверкуЗаполненияНаСервере()
	ОбязательныеПоля = ns_ЛичныеВещиВалидация.ПолучитьОбязательныеПоля();
	Для каждого ИмяПоля Из ОбязательныеПоля Цикл
		Элементы[ИмяПоля].АвтоОтметкаНезаполненного = Истина;
	КонецЦикла; 
	
	ДополнительныеОбязательныеПоля = ns_ЛичныеВещиВалидация.ПолучитьДополнительныеОбязательныеПоля(Объект.Category);
