...
Функция ОчиститьМакетСклада()
	
	Если НЕ Владелец.КартаСклада.Получить() = Неопределено Тогда
		ФормаЭлементаСклада = Владелец.ПолучитьФорму("ФормаЭлемента");
		Если ФормаЭлементаСклада.Открыта() Тогда
			ВладелецОбъект = ФормаЭлементаСклада.ЭтотОбъект;
		Иначе
		...
		Исключение
			Сообщить("Невозможно выполнить очистку макета карты склада! Склад """ + СокрЛП(Строка(ВладелецОбъект)) + """ заблокирован.");
			Возврат Истина;
		КонецПопытки;
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		Если Владелец.ВидСклада = Перечисления.ВидыСкладов.Ячеистый Тогда
			ОбязательныеРеквизиты.Вставить("ШиринаЯчейки",1);
			ОбязательныеРеквизиты.Вставить("ДлинаЯчейки",1);
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	Если Заполнение и обЗначениеНеЗаполнено(ЭтотОбъект.Код) Тогда
		Результат=Ложь;
		Ошибки=Ошибки+"Реквизит ""Номер ячейки"" не заполнен !"+Символы.ПС;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Отказ = Отказ ИЛИ ОчиститьМакетСклада();
