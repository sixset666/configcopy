...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	Если ДляЭлемента Тогда
		ОбязательныеРеквизиты.Вставить("ПредставлениеНаМобильномУстройстве", 3);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Порядок = 0 Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	МАКСИМУМ(КомандыМобильногоПриложения.Порядок) КАК Порядок
		|ИЗ
...
Процедура ПередУдалением(Отказ)
	...
	//Сдвинем все нижние элементы в сортировке
	//Запрос=Новый Запрос("ВЫБРАТЬ
	//                    |	ВидыСостоянийЗаказНарядов.Ссылка КАК ВидыСостоянийЗаказНарядовСсылка
	//                    |ИЗ
	//                    |	Справочник.ВидыСостоянийЗаказНарядов КАК ВидыСостоянийЗаказНарядов
	//                    |ГДЕ
	//                    |	ВидыСостоянийЗаказНарядов.Порядок > &Порядок
	//                    |
	//                    |УПОРЯДОЧИТЬ ПО
	...
	//	СправочникОбъект=ЗапросВыборка.ВидыСостоянийЗаказНарядовСсылка.ПолучитьОбъект();
	//	СправочникОбъект.Порядок=СправочникОбъект.Порядок-1;
