...
Перем КопироватьСодержимое; // В новый элемент будет записано содержимое копируемого при записи
Перем КопируемыйОбъект;   	// Объект копирования при создании элемента с помощью копирования
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	#Если Клиент Тогда
		КопироватьСодержимое = Вопрос("Скопировать содержимое прайса """+Строка(ОбъектКопирования)+""" в новый прайс-лист?",РежимДиалогаВопрос.ОКОтмена,25,КодВозвратаДиалога.Отмена)=КодВозвратаДиалога.ОК;
	#Иначе
		КопироватьСодержимое = Ложь;
	#КонецЕсли
	КопируемыйОбъект     = ОбъектКопирования;
...
Процедура ПриЗаписи(Отказ)
	...
	Если КопироватьСодержимое Тогда
		...
		ПараметрыЗаполнения.Вставить("ВидПрайсЛиста",КопируемыйОбъект.Ссылка);
		Справочники.ПрайсЛист.СоздатьЭлемент().ЗаполнитьПрайсЛист(ЭтотОбъект.Ссылка,3,ПараметрыЗаполнения);
	КонецЕсли;		
