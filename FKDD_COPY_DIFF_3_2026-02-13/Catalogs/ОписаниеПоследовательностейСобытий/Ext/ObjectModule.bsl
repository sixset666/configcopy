// Модуль справочника Описание последовательностей событий
...
Функция ПолучитьОбязательныеРеквизиты(ДляЭлемента=Истина, ДляГруппы=Ложь) Экспорт
	...
	ОбязательныеРеквизиты.Вставить("ВидСобытия", 3);
	
	ОбязательныеОчередныеСобытия = Новый Структура;
	ОбязательныеОчередныеСобытия.Вставить("ВидСобытия", 3);
	
	ОбязательныеРеквизиты.Вставить("ОчередныеСобытия", ОбязательныеОчередныеСобытия);
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	//В колонке "При наличии не может быть указан вид события, для которого заполняется ТЧ"
	Если НЕ МожетБытьПервым Тогда
		ОшибочнаяСтрока = ОчередныеСобытия.Найти(ВидСобытия,"ПриНаличии");
		Если ОшибочнаяСтрока <> Неопределено Тогда
			дкДобавитьОшибку(Ошибки, НСтр("ru='Вид события ""'") + ВидСобытия + НСтр("ru='"" не может быть указан в колонке ""При наличии""'"));
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
	Возврат Результат;
