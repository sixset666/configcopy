...
Функция План() Экспорт
	...
	Результат = Истина;
	// Получим документ, в зависимости от планируемого поступления или списания ДС, по которому будем делать записи в регистр
	Запрос = Новый Запрос;
...
Функция ПланПеремещение() Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ПланируемоеДвижениеДС.ДатаПлатежа КАК Период,
	               |	ПланируемоеДвижениеДС.СуммаДокумента КАК Расход,
	               |	0 КАК Поступление
	               |ИЗ
	               |	Документ.ПеремещениеДенежныхСредствПлан КАК ПланируемоеДвижениеДС
	               |ГДЕ
	               |	ПланируемоеДвижениеДС.Ссылка = &Ссылка
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               ...
	               |	ПланируемоеДвижениеДС.СтатьяДДСПроступление,
...
Функция ФактПеремещение() Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
				   ...
				   //|	ПланируемоеДвижениеДС.ДокументПланирования
				   |ИЗ
