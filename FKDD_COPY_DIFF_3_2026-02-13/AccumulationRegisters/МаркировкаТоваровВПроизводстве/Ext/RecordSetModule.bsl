...
Функция ПолучитьТаблицуТоваров()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ДокументКодыМаркировки.КодМаркировкиBase64 КАК КодМаркировкиBase64
...
//
Функция ПроверитьОстаткиМаркировкиВПроизводстве(ТаблицаМаркировок, МоментВремени = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("МоментВремени", ?(МоментВремени = Неопределено, Новый МоментВремени(КонецДня(ТекущаяДата())), МоментВремени));
...
Процедура ДополнитьТаблицуРезультатКодамиМаркировки()
	...
	Для Каждого ТекущаяСтрока Из РезультатЗапросаПоТоварам Цикл
		...
		ТекущаяСтрока.GTIN = СтруктураМаркировки.ДанныеШтрихкода["01"].Значение;
		ТекущаяСтрока.СерийныйНомер = СтруктураМаркировки.ДанныеШтрихкода["21"].Значение;
...
Функция Приход() Экспорт
	...
	ВсеОК = НЕ РегистрыСведений.СостоянияКодовМаркировки.КодМаркировкиЗапрещенДляВывода(РезультатЗапросаПоТоварам, ДокументОбъект, РежимПроведения);
	...
	Для каждого СтрокаТоваров Из РезультатЗапросаПоТоварам Цикл
		...
		НоваяЗапись.КодМаркировкиBase64 = СтрокаТоваров.КодМаркировкиBase64;
...
Функция Расход() Экспорт
	...
	ЗапросПоМаркировке.Текст = "ВЫБРАТЬ
	...
	ЗапросПоМаркировке.УстановитьПараметр("ХарактеристикаНоменклатуры",РезультатЗапросаПоТоварам.ВыгрузитьКолонку("ХарактеристикаНоменклатуры"));
	...
	СтруктураОтбораТоваров=Новый Структура("ЗаказНаряд,Цех,Номенклатура,ХарактеристикаНоменклатуры,GTIN,СерийныйНомер");
	...
	Для каждого СтрокаТоваров Из РезультатЗапросаПоТоварам Цикл
		...
		Если ВыборкаПоМаркировкеПриход.НайтиСледующий(СтруктураОтбораТоваров) Тогда
			КоличествоМаркировок=КоличествоМаркировок+?(ВыборкаПоМаркировкеПриход.Количество=NULL,0,ВыборкаПоМаркировкеПриход.Количество);
		...
		Если ЗапросПоМаркировкеРасход.НайтиСледующий(СтруктураОтбораТоваров) Тогда
			КоличествоМаркировок=КоличествоМаркировок-?(ЗапросПоМаркировкеРасход.Количество=NULL,0,ЗапросПоМаркировкеРасход.Количество);
			ГТД = ВыборкаПоМаркировкеПриход.ГТД;
		КонецЕсли;
		Для Каждого СтрокаТекущихПеремещений Из ЭтотОбъект Цикл
			Если СтрокаТекущихПеремещений.Номенклатура=СтрокаТоваров.Номенклатура И 
				 ...
				 СтрокаТекущихПеремещений.ЗаказНаряд=ТекЗаказНаряд И
				 СтрокаТекущихПеремещений.КодМаркировки=СтрокаТоваров.КодМаркировки Тогда
		...
		Если Количество>КоличествоМаркировок Тогда
			...
			Если обЗначениеНеЗаполнено(СтрокаТоваров.ХарактеристикаНоменклатуры) Тогда
				дкСообщитьРезультат("Товар <"+СокрЛП(СтрокаТоваров.Номенклатура)+"> с маркировкой <"+СокрЛП(СтрокаТоваров.КодМаркировки)+"> не перемещался в производство.",, ДокументОбъект);
			Иначе
				дкСообщитьРезультат("Товар <"+СокрЛП(СтрокаТоваров.Номенклатура)+"> с характеристикой <"+СокрЛП(СтрокаТоваров.ХарактеристикаНоменклатуры)+"> с маркировкой <"+СокрЛП(СтрокаТоваров.КодМаркировки)+"> не перемещался в производство",, ДокументОбъект);
			КонецЕсли; 
			ВсеОК=Ложь;
		КонецЕсли; 
