// Модуль набора записей регистра "ПрочиеАктивыВЭксплуатации"
...
Перем ДатаДвижений Экспорт;				  //Если неопределенна, то берется дата документа
//
...
//
Функция ПолучитьОстаткиАктивов(ВыбПодразделение = Неопределено, ВыбМОЛ = Неопределено, ВыбАктив = Неопределено, Момент = Неопределено) Экспорт
...
Функция ПроверитьНачислениеАмортизации(ВыбАктив)
	...
	|	ПрочиеАктивыВЭксплуатации.ПрочийАктив = &Актив И НЕ 
	|	ПрочиеАктивыВЭксплуатации.Регистратор = &Регистратор И 
	...
	|	(ТаблицаАктива.ДатаВводаВЭксплуатацию МЕЖДУ &НачПериода И &КонПериода ИЛИ 
	|	 ТаблицаАктива.ДатаВводаВЭксплуатацию = ДатаВремя(1, 1, 1))))";
...
Функция Приход() Экспорт
	...
	Для каждого ТекАктив Из РезультатЗапросаПоАктивам Цикл			
		...
		Если НЕ (ТекАктив.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ
			     ТекАктив.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ
				 ТекАктив.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Инструменты) Тогда			
			...
			Если ЭтоПервыйВвод И ВыборкаДвиженийАктивов.НайтиСледующий(Новый Структура("ПрочийАктив",ТекАктив.Актив)) Тогда
				дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Уже был в эксплуатации", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
				ВсеОК = Ложь;
			КонецЕсли;
		КонецЕсли;
		Если ЭтоПервыйВвод
			...
			дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Способ начисления амортизации <По объему выработки> не соответствует виду актива <" + ТекАктив.Актив.ВидПрочегоАктива + ">", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
			ВсеОК = Ложь;
		КонецЕсли;	
		...
		Если ТекАктив.ТипЭксплуатации.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизации.ПоСуммеЧиселЛет И Цел(ТекАктив.Актив.СрокПолезногоИспользования/12) <> ТекАктив.Актив.СрокПолезногоИспользования/12 Тогда
			дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Имеет неверный срок полезного использования (нецелое число лет)", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
			ВсеОК = Ложь;
		КонецЕсли;
		...
		Если ЭтоАмортизация И ПроверитьНачислениеАмортизации(ТекАктив.Актив) Тогда
			дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Амортизацию в этом месяце начислять не требуется", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
			ВсеОК = Ложь;
		КонецЕсли;
								
		//МОЛ = ?(МОЛ = Неопределено, ТекАктив.МОЛ, МОЛ);
		...
		Если ВсеОК Тогда
			...
			НоваяЗапись.БалансоваяСтоимость    = Окр(обПересчет(ТекАктив.БалансоваяСтоимость, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаРегл, КурсРегл), 2);
			НоваяЗапись.БалансоваяСтоимостьУпр = Окр(обПересчет(ТекАктив.БалансоваяСтоимость, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаУпр, КурсУпр), 2);
			НоваяЗапись.СуммаАмортизации      = Окр(обПересчет(СуммаАмортизации_, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаРегл, КурсРегл), 2);
			НоваяЗапись.СуммаАмортизацииУпр   = Окр(обПересчет(СуммаАмортизации_, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаУпр, КурсУпр), 2);
			НоваяЗапись.СуммаОбслуживания     = Окр(обПересчет(ТекАктив.СуммаОбслуживания, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаРегл, КурсРегл), 2);
			НоваяЗапись.СуммаОбслуживанияУпр  = Окр(обПересчет(ТекАктив.СуммаОбслуживания, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаУпр, КурсУпр), 2);
		...
		Если ВсеОК И ЭтоПервыйВвод И ТекАктив.ТипЭксплуатации.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизации.СтопроцентныйПриВводе Тогда
			...
			НоваяЗапись.СуммаАмортизации      = Окр(обПересчет(ТекАктив.БалансоваяСтоимость, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаРегл, КурсРегл), 2);
			НоваяЗапись.СуммаАмортизацииУпр   = Окр(обПересчет(ТекАктив.БалансоваяСтоимость, ДокументОбъект.ВалютаДокумента, ДокументОбъект.КурсДокумента, ВалютаУпр, КурсУпр), 2);
		...
		Если ЭтоПервыйВвод И ВсеОК Тогда
			...
			Актив_.ПервоначальнаяСтоимость = обПересчет(ТекАктив.БалансоваяСтоимость, ДокументОбъект.ВалютаДокумента, ДокументОбъект.Дата, Константы.ВалютаУправленческогоУчетаКомпании.Получить(), ?(обЗначениеНеЗаполнено(ДокументОбъект.КурсВалютыУпр), ДокументОбъект.Дата, ДокументОбъект.КурсВалютыУпр));
			...
			Попытка
				Актив_.Записать();
			Исключение
				дкСообщитьРезультат("Не удалось перезаписать актив """ + СокрЛП(Актив_) + """!" + ОписаниеОшибки(), "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
				ВсеОК = Ложь;
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
...
Функция Расход() Экспорт
	...
	Для каждого ТекАктив Из РезультатЗапросаПоАктивам Цикл
		...
		Если СтрокаАктива = Неопределено Тогда //такого актива нет в эксплуатации
			дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Подразделение """ + СокрЛП(ПодразделениеКомпании) + """. Не обнаружен", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
			ВсеОК = Ложь;
		ИначеЕсли СтрокаАктива.Количество < ТекАктив.Количество Тогда //не хватает такого актива по количеству в эксплуатации
			дкСообщитьРезультат("Прочий актив """ + СокрЛП(ТекАктив.Актив) + """. Остаток " + СтрокаАктива.Количество + ". Списывается " + ТекАктив.Количество +". Превышение " + (ТекАктив.Количество - СтрокаАктива.Количество) + ".", "Важное&Прочие активы в эксплуатации:", ДокументОбъект, ТекАктив.Актив);
			ВсеОК = Ложь;	
		КонецЕсли;
