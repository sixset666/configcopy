...
Процедура ОбработкаТаблицыЗаголовка(ТаблицаЗаголовка) Экспорт
	...
	СтрокаПоиска = ТаблицаЗаголовка.Найти("ИмяОтчета", "ИмяСтроки");
	Если СтрокаПоиска=Неопределено Тогда Возврат; КонецЕсли;
	
	НоваяСтрока = ТаблицаЗаголовка.Вставить(ТаблицаЗаголовка.Индекс(СтрокаПоиска));
