...
Перем ДеревоДоступныхПолей Экспорт;                     //хранит дерево полей
...
Перем мПериодичность Экспорт;                           //Периодичность
Перем ТаблицаДляВывода;                                 //хранит таблицу для вывода
Перем СтруктураИтоговыхВыраженийПоказателей Экспорт;    //хранит структуру несвязанных показателей
...
Функция ПолучитьТаблицуИсточникаДанных(ТекПостроитель, ТаблицаВывода, СтруктураКолонок) Экспорт
	...
	Выборка = ТекПостроитель.Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Покупатель", "Все");
	...
	СтруктураНеобходимыхПоказателей = Новый Структура ("КоэффициентВариации, СебестоимостьУпр, СуммаНаценкиУпр");
...
Функция СформироватьТабличныйДокумент(ДокументРезультат=Неопределено) Экспорт
	...
	Если ИзмеренияСтроки.Найти("XYZГруппа") = Неопределено Тогда
		СтрокаДерева	= ДеревоДоступныхПолей.Строки.Найти("XYZГруппа", "ПутьКДанным");
		НоваяСтрока		= ИзмеренияСтроки.Вставить(СтрокаДерева.ПутьКДанным, СтрокаДерева.Имя, ТипИзмеренияПостроителяОтчета.Элементы, , , 0);
		НоваяСтрока.Представление = СтрокаДерева.Представление;
	КонецЕсли;
	
	Если ИзмеренияСтроки.Найти("Покупатель") = Неопределено Тогда
		СтрокаДерева = ДеревоДоступныхПолей.Строки.Найти("Покупатель", "ПутьКДанным");
		НоваяСтрока = ИзмеренияСтроки.Добавить();

		НоваяСтрока.ПутьКДанным		= СтрокаДерева.ПутьКДанным;
		НоваяСтрока.ТипИзмерения	= ТипИзмеренияПостроителяОтчета.Элементы;
		НоваяСтрока.Представление	= СтрокаДерева.Представление;
	КонецЕсли;
...
Функция СформироватьДиаграмму(ПолеДиаграммы=Неопределено, СтруктураПараметров=Неопределено) Экспорт
	...
	СтруктураПараметров.Серии = "XYZГруппа";
	СтруктураПараметров.Точки = "Покупатель";
