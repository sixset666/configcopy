...
Перем СтруктураИтоговыхВыраженийПоказателей Экспорт;   // структура итоговых выражений показателей
Перем ШаблонИсточникаДанных Экспорт;                   // шаблон источника данных
Перем СтруктураТипаКолонки;                            // структура типа колонки
Перем ПоследовательностиОтчета Экспорт;                // последовательности отчета
...
Функция ПолучитьТаблицуИсточникаДанных(ТекПостроитель, ТаблицаВывода, ПоляНастройкиОтчета) Экспорт
	...
	Для Каждого ЭлементОтбора Из ПостроительОтчета.Отбор Цикл
		...
		Иначе
			...
			ТекстУсловия = СтрЗаменить(отСформироватьСтрокуВидаСравнения(ЭлементОтбора.ВидСравнения),"&","&"+ЭлементОтбора.Имя);
			...
			ИначеЕсли Найти(ПутьКДанным,"Подразделение")=1 Тогда
				ПутьКДанным = "СкладКомпании."+ПутьКДанным;
			КонецЕсли;
			СтрокаУсловия=СтрокаУсловия+Символы.ПС+?(НЕ ПустаяСтрока(СтрокаУсловия)," И ","")+СтрЗаменить(ТекстУсловия,"#", ПутьКДанным);
			СтрокаУсловияДвижений=СтрокаУсловияДвижений+" И "+Символы.ПС+СтрЗаменить(ТекстУсловия,"#", "ОстаткиАвтомобилей."+ПутьКДанным);
		КонецЕсли;
	...
	|			ИЗ
	|				РегистрНакопления.ОстаткиАвтомобилей.Остатки(&ДатаНач,"+СтрокаУсловия+" ) КАК ОстаткиАвтомобилейОстатки
	|			
	|			ОБЪЕДИНИТЬ ВСЕ
	|			
	|			ВЫБРАТЬ
	...
	|				ВЫБОР
	|					КОГДА ОстаткиАвтомобилей.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|						ТОГДА ОстаткиАвтомобилей.Количество
	|					ИНАЧЕ -ОстаткиАвтомобилей.Количество
	|				КОНЕЦ,
	...
	Для Счетчик=1 По КоличествоСтрок Цикл
		...
		Выборка[Счетчик].Количество = Выборка[Счетчик].Количество+Выборка[Счетчик-1].Количество;
	КонецЦикла;
...
Функция СформироватьДиаграмму(ПолеДиаграммы=Неопределено, СтруктураПараметров=Неопределено) Экспорт
	...
	Если ПолеДиаграммы.ТипДиаграммы = ТипДиаграммы.Круговая ИЛИ ПолеДиаграммы.ТипДиаграммы = ТипДиаграммы.КруговаяОбъемная Тогда
		Предупреждение("Отчет динамика остатков стоимости автомобилей не может использоваться с типом диаграммы """+СокрЛП(ПолеДиаграммы.ТипДиаграммы)+"""");
