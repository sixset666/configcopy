// Модуль документа ЗаявкаНаРасходДС
...
Функция ПроверитьКорректность(Ошибки="", ДопРеквизиты=Неопределено, Заполнение=Истина, Уникальность=Истина) Экспорт
	...
	Если обЗначениеНеЗаполнено(СтруктурнаяЕдиница) Тогда
		Если ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			дкДобавитьОшибку(Ошибки, "Не выбран банковский счет заявки на расходование денежных средств.");
		Иначе
			дкДобавитьОшибку(Ошибки, "Не выбрана касса заявки на расходование средств.");
		КонецЕсли;	
		Результат = Ложь;		
	КонецЕсли; 
...
Функция ОбработкаРеквизита(Имя,ТекСтрока=Неопределено,ЭтаФорма=Неопределено,ДопПараметры=Неопределено) Экспорт
	...
	ИначеЕсли Имя="Контрагент" Тогда
		...
		КонецЕсли;
		//KamikadZe begin add 07.08.2020 21:47:19
		Если НЕ Контрагент.Пустая() Тогда
			Если (СчетКонтрагента.Пустая()) ИЛИ (НЕ СчетКонтрагента.Владелец = Контрагент) Тогда
	...
	//KamikadZe begin add 04.10.2017 21:36:59
	ИначеЕсли Имя="Платежи.СуммаДокумента" ИЛИ Имя="Платежи.СтавкаНДС" Тогда
...
Функция ПолучитьСписокХозОпераций() Экспорт
	...
	СписокХозОпераций.Добавить("ЗаявкаНаРасходСРС"   ,"С расчетного счета",Истина);
	СписокХозОпераций.Добавить("ЗаявкаНаРасходИзКассы","Из кассы",Ложь);	
...
//
Функция ПечатьЗаявкаНаРасходДС(ТабДокумент) Экспорт
		
	Макет = ПолучитьМакет("ЗаявкаНаРасходДС");
	...
	Если ЭтотОбъект.ХозОперация = Справочники.ХозОперации.ЗаявкаНаРасходИзКассы Тогда
	...
	ОбластьПодвал.Параметры.СуммаПрописью = "Всего планируется израсходовать сумму " + обЧислоПрописью(СуммаВсего,ЭтотОбъект.ВалютаДокумента);
...
// Процедуры - обработчик ввода на основании
Процедура ОбработкаЗаполнения(Основание)
	...
	ДопНазначение = "";	
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.СчетОтПоставщика") ИЛИ
		ТипЗнч(Основание) = Тип("ДокументСсылка.СчетОтПоставщикаЗаАвтомобили") Тогда
		СтруктурнаяЕдиница = Основание.РасчетныйСчетКонтрагента; 
	КонецЕсли;
	
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.СчетОтПоставщика") ИЛИ 
		 ТипЗнч(Основание) = Тип("ДокументСсылка.СчетОтПоставщикаЗаАвтомобили") ИЛИ 
		 ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПоставщику") ИЛИ   
		 ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплату") ИЛИ 
		 ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПоставщикуНаАвтомобиль") Тогда
	ИначеЕсли
		 ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеАвтомобилей") Тогда
		 НовыйПлатеж.СтавкаНДС = Основание.Автомобили[0].СтавкаНДС;     
		 НовыйПлатеж.СуммаНДС = Основание.Автомобили.Итог("СуммаНДС");  
		 ДопНазначение = " вх. номер " + Основание.ВхДокНомер + " вх. дата " + Формат(Основание.ВхДокДата, "ДФ=dd.MM.yyyy") + " сумма:"+ Основание.СуммаДокумента + " в том числе НДС:" + НовыйПлатеж.СуммаНДС  
	 ИначеЕсли
		 ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваров") Тогда
		 НовыйПлатеж.СтавкаНДС = Основание.Товары[0].СтавкаНДС;     
		 НовыйПлатеж.СуммаНДС = Основание.Товары.Итог("СуммаНДС"); 
 		 ДопНазначение = " вх. номер " + Основание.ВхДокНомер + " вх. дата " + Формат(Основание.ВхДокДата, "ДФ=dd.MM.yyyy") + " сумма:"+ Основание.СуммаДокумента + " в том числе НДС:" + НовыйПлатеж.СуммаНДС  
	КонецЕсли;

	Контрагент            = Основание.Контрагент;
	ДоговорВзаиморасчетов = Основание.ДоговорВзаиморасчетов;     
	Назначение = "Оплата по договору "+ ДоговорВзаиморасчетов.Наименование+ ДопНазначение;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//->Diginova _Акт сверки КСО_ 21.12.2021
	Если ПометкаУдаления тогда
		УбираемЗаписиИзРегистра();	
	КонецЕсли;
	//<-Diginova _FlaiM_	
	Если НЕ дкПередЗаписью(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения, , , ,"10111") Тогда Возврат; КонецЕсли;  
	
	
	//<<ЧалапАю БизТех 09.11.2023 - проверим доступность настроек адресатов
	...
	ЗапросАдресатов.УстановитьПараметр("Роль"		, Перечисления.БТ_РолиАдресатовЗаявок.СотрудникЦФО);
	...
	Если Не РезультатАдресатов.Пустой() Тогда
		...
		Пока ВыборкаАдресатов.Следующий() Цикл
			...
			Если НастройкиАдресата.Отбор.Элементы.Количество()>0 Тогда
				//Если НастройкиАдресата.Отбор.Элементы[0].ПравоеЗначение.НайтиПоЗначению(ЭтотОбъект.ПодразделениеКомпании) = Неопределено тогда //БизТех Аляль 09.01.2024г. выдавало ошибку 
				Если НЕ НастройкиАдресата.Отбор.Элементы[0].ПравоеЗначение = ЭтотОбъект.ПодразделениеКомпании тогда
				//	НеНайденЦФО = Истина;  //БизТех надо переписать!
				КонецЕсли;
			Иначе 
				//НеНайденЦФО = Истина;
			КонецЕсли;
		КонецЦикла; 
		Если НеНайденЦФО тогда
			Сообщить("На роль " + Перечисления.БТ_РолиАдресатовЗаявок.СотрудникЦФО + " для подразделения " + ЭтотОбъект.ПодразделениеКомпании + " не найдено адресатов. Проведение невозможно!");
			Отказ = Истина;
		КонецЕсли;

	КонецЕсли;
	//ЧалапАю БизТех 09.11.2023>> 
	//Аляль БизТех 11.01.2024г. временно для тестовой эксплуатации <<
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		ЭтотОбъект.СтатусТТ = Перечисления.ТТ_СтатусыЗаявкиНаРасходДС.Оплата;
	КонецЕсли; 
	//>>
...
Процедура ПриЗаписи(Отказ)
	...
	Если РегистрыСведений.БТ_СтатусыЗаявкиДДС.УзнатьТекСтатус(Ссылка) = Неопределено Тогда
		
		РегистрыСведений.БТ_СтатусыЗаявкиДДС.УстановитьСтатус(Ссылка, Перечисления.БТ_СтатусыЗаявки.Подготовлено);
...
//KamikadZe begin add 05.10.2017 21:38:14
...
//->Diginova _Акт сверки КСО_ 21.12.2021
Функция УбираемЗаписиИзРегистра()
	 	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	DN_АВ_КредитноСтраховыеСделки.Регистратор КАК Регистратор,
		...
		|ГДЕ
		|	DN_АВ_КредитноСтраховыеСделки.Период МЕЖДУ &ДатаНачала И &ДатаОкончания
		|	И DN_АВ_КредитноСтраховыеСделки.ЗаявкаНаРасходДС = &ЗаявкаНаРасходДС";
	
	Запрос.УстановитьПараметр("ДатаНачала", ДобавитьМесяц(НачалоДня(ТекущаяДата()),-3));
	Запрос.УстановитьПараметр("ДатаОкончания", КонецДня(ТекущаяДата()));
	Запрос.УстановитьПараметр("ЗаявкаНаРасходДС", Ссылка);
	...
	Пока Выборка.Следующий() Цикл 
		...
		Записи.Отбор.Регистратор.Установить(Выборка.Регистратор);
		Записи.Прочитать();
		
		Для каждого Запись из Записи цикл 
			Запись.ЗаявкаНаРасходДС = Документы.DN_АктСверкиКредитСтрах.ПустаяСсылка(); 
			АктСверки = Выборка.АктСверки.ПолучитьОбъект();
			АктСверки.ЗаявкаНаРасходДС = Документы.DN_АктСверкиКредитСтрах.ПустаяСсылка();
			АктСверки.Записать();			
		КонецЦикла;
...
КонецФункции // УбираемЗаписиИзРегистра()
//<-Diginova _FlaiM_
