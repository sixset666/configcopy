...
Функция СписокКонстантныхЗначенийОбработки()
	...
	КонсЗнач.Вставить("РегистрСопоставления",РегистрыСведений.БТ_СопоставлениеДанныхHavalСтаврополь);
	КонсЗнач.Вставить("РегистрСопоставленияВидовРемонта",РегистрыСведений.БТ_СопоставлениеВидовРемонтаHavalСтаврополь);
	КонсЗнач.Вставить("РегистрСопоставленияПодразделений",РегистрыСведений.БТ_СопоставлениеПодразделенийHavalСтаврополь);
	КонсЗнач.Вставить("КонтрагентПоУмолчанию", Справочники.Контрагенты.НайтиПоКоду("ЦБ000440"));
	КонсЗнач.Вставить("IDЦехКузовногоРемонта", 5);
	КонсЗнач.Вставить("IDЦехНекузовногоРемонта", 2);
	КонсЗнач.Вставить("РазбивкаВидовРемонтаПоТО", Ложь);
	КонсЗнач.Вставить("ОбрабатыватьСогласияПриВыгрузкеЗН", Истина);
	Возврат КонсЗнач; 
