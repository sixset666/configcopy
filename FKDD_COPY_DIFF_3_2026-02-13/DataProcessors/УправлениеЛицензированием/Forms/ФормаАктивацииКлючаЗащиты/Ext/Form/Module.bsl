...
Процедура КомандаАктивироватьВручную(Команда)
	...
	ПотокЧтения.Прочитать(ИмяФайлаОтветаАктивации,КодировкаТекста.ANSI);
	...
	Иначе
		...
		Иначе
			ЭтаФорма.Закрыть(ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаСохранитьФайл(Команда)
	...
	ТекстПредупреждения = "Запрос активации сохранён в файл: " + ИмяФайла +".
	|Отправьте этот файл по электронной почте на адрес """ + ЛицензированиеСервер.АдресЭлектроннойПочтыДляАктивации(ИмяМакетаПараметровЛицензирования) + """,
	|получите ответ и загрузите его на странице загрузки ответа активации";
	Предупреждение(ТекстПредупреждения,,"Создан файл запроса активации");
...
Процедура АдресСервераНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыбранныйЭлемент = СписокСерверов.ВыбратьЭлемент("Выберите сервер лицензирования");
...
Процедура ИмяФайлаОтветаАктивацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//Иначе
	//	ОписаниеОшибки = "Ошибка открытия файла. Попробуйте открыть через тонкий или толстый клиент.";
	//КонецЕсли;
КонецПроцедуры
