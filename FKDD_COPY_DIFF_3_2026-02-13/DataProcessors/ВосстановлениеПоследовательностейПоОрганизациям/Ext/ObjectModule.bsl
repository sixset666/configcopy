...
Функция ПолучитьМинимальнуюДатуГраниц(СтруктураПоследовательностей=Неопределено, Организация)
	...
	Для Каждого ТекПоследовательность Из ТаблицаПоследовательностей Цикл
		...
		КонецЕсли;
		//anton
		Если ТекПоследовательность.ИмяГраницы <> "ГраницыЗаказыНаАвтомобили" И ТекПоследовательность.ИмяГраницы <> "ГраницыКомплектация" Тогда 
			ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "//$УсловиеОрганизация$", "ГДЕ $ИмяГраницы$Границы.Организация=&Организация");
		КонецЕсли;
		//anton
...
Функция ПолучитьТекстЗапросаРегистровНакопления(СтруктураПоследовательностей, МинимальнаяДата = '00010101', ПоТекДату = '00010101')
	...
	Для Каждого ТекПоследовательность Из ТаблицаПоследовательностей Цикл
		...
		//anton
		Если ТекПоследовательность.ИмяГраницы <> "ГраницыЗаказыНаАвтомобили" И ТекПоследовательность.ИмяГраницы <> "ГраницыКомплектация" Тогда 
			Если обЗначениеНеЗаполнено(СтрокаУсловия) Тогда
				СтрокаУсловия=СтрокаУсловия+ " ГДЕ $ИмяТаблицы$.Регистратор.Организация =&Организация ";
			Иначе
				СтрокаУсловия=СтрокаУсловия+ " И $ИмяТаблицы$.Регистратор.Организация =&Организация ";		
			КонецЕсли;		
		КонецЕсли;		
		//anton
...
// Удаление границ
//
Процедура УдалитьАктуальныеГраницы(СтруктураПоследовательностей=Неопределено, Организация) Экспорт
	...
	Запрос = Новый Запрос();
	МинимальнаяДата = ПолучитьМинимальнуюДатуГраниц(СтруктураПоследовательностей, Организация);
	//anton
	Запрос.УстановитьПараметр("Организация", Организация);
	//anton
	...
	Для Каждого ТекПоследовательность Из ТаблицаПоследовательностей Цикл
		...
		Для Каждого ТекИзмерение Из СтруктураИзмерений Цикл
			...
			Иначе
				...
				//anton
				СтрокаСгруппировать =СтрокаСгруппировать+ ?(СтрокаСгруппировать="","",","+Символы.ПС)+"	"+ИмяГраницы+"Границы."+ИмяИзмерения;
				//anton
		КонецЕсли;
	...
	//anton	
	|ГДЕ
	|(ИтоговаяТаблица.Организация.Ссылка ЕСТЬ NULL ИЛИ ИтоговаяТаблица.Организация = &Организация)
	|
	//anton
	|ИТОГИ
	|ПО
...
//
Функция ПолучитьТаблицуНеактуальныхДокументов(ПоТекДату = '00010101', Запрос = Неопределено, СтруктураПоследовательностей=Неопределено, Организация) Экспорт
...
//
Функция Восстановить(СписокПоследовательностей, ПоТекДату = '00010101', Организация) Экспорт
	...
	Иначе
		ТаблицаДокументов = ПолучитьТаблицуНеактуальныхДокументов(ПоТекДату, Запрос, СписокПоследовательностей, Организация);
	КонецЕсли;
	...
	Иначе
		...
		Пока ИСТИНА Цикл
			...
			Для Каждого ТекСтрока Из ТаблицаДокументов Цикл
				//anton
				//Если ЗначениеЗаполнено(Организация) И ТекСтрока.Документ.Организация <> Организация Тогда
				//	Документ = Неопределено;
				//	Продолжить;
				//КонецЕсли;
				//anton
			...
			Попытка
				...
				Для Каждого ТекЭлемент Из СписокПоследовательностей Цикл
					...
					Если ТаблицаИзмерений.Количество()>0 Тогда
						...
						Для Каждого Измерение Из ТекПоследовательность.Измерения Цикл
							//anton
							//Если (ТекПоследовательность.ИмяГраницы <> "ГраницыЗаказыНаАвтомобили" 
							//	И ТекПоследовательность.ИмяГраницы <> "ГраницыКомплектация") 
							//	Тогда
	...
	Если СписокПоследовательностей.Количество()>0 Тогда
		УдалитьАктуальныеГраницы(СписокПоследовательностей, Организация);
	КонецЕсли;
...
СписокРегистровНакопления.Добавить(Новый Структура("СкладКомпании, Организация","СкладКомпании","Регистратор.Организация"), "ПартииТоваровКомпании");
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("ДоговорВзаиморасчетов,Организация","ДоговорВзаиморасчетов","Регистратор.Организация"), "ВзаиморасчетыКомпании");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("СкладКомпании,Организация","СкладКомпании","Регистратор.Организация"), "ОстаткиТоваровОрдерныйСклад");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
МассивИзмерений = Новый Массив(); МассивИзмерений.Добавить("СкладКомпании");
//anton
МассивИзмерений.Добавить("Организация");
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("Заказ,Организация","Заказ","Регистратор.Организация"), "ЗаказыПокупателей");
СписокРегистровНакопления.Добавить(Новый Структура("Заказ,Организация","ЗаказПоставщику","Регистратор.Организация"), "ЗаказыПоставщикам");
СписокРегистровНакопления.Добавить(Новый Структура("Заказ,Организация","ЗаказПокупателя","Регистратор.Организация"), "ЗаказыРаспределение");
СписокРегистровНакопления.Добавить(Новый Структура("Заказ,Организация","ЗаказПоставщика","Регистратор.Организация"), "ЗаказыРаспределение");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("ЗаказНаряд,Организация","ЗаказНаряд","Регистратор.Организация"), "ТоварыВПроизводстве");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("СкладКомпании,Организация","СкладКомпании","Регистратор.Организация"), "ОстаткиАвтомобилей");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
МассивИзмерений = Новый Массив(); МассивИзмерений.Добавить("СкладКомпании");
//anton
МассивИзмерений.Добавить("Организация");
...
СписокРегистровНакопления = Новый СписокЗначений;
СписокРегистровНакопления.Добавить(Новый Структура("СкладКомпании,Организация","СкладКомпании","Регистратор.Организация"), "ОстаткиАвтомобилейОрдерныйСклад");
НоваяСтрока.РегистрыНакопления = СписокРегистровНакопления;
МассивИзмерений = Новый Массив(); МассивИзмерений.Добавить("СкладКомпании");
//anton
МассивИзмерений.Добавить("Организация");
