...
Функция ПолучитьПараметрыСессии(Параметры = Неопределено) Экспорт
	...
	Если НЕ ПараметрыСессии = Неопределено Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации("Смещение UTC", УровеньЖурналаРегистрации.Информация,,, Строка(ПараметрыСеанса.смсКорректировкаUTC/60/60) + " ч.");
		КонецЕсли;
	Иначе
		КодОшибки = -99999;
...
Функция ПолучитьСписокНомеровИзСтроки(НомерОтправителя) Экспорт
	...
	Пока НЕ ПустаяСтрока(ТекстКонстанты) Цикл
		...
		Иначе // если больше одного
			...
			//ЛевСтрокаНомера = СокрЛП(ЛевСтрокаНомера);
			// Добавляем в список
...
КонецФункции // ОписаниеОшибокВебСервиса()
...
Функция ПодключитьВебСервис()
	...
	Попытка
		...
		Иначе
			// Резервный сервер
			ВебСервис = WSСсылки.SMS4BРезерв.СоздатьWSПрокси("SMS client", "WSSM", "WSSMSoap12");
		КонецЕсли; 
	Исключение
		Возврат Неопределено;
	КонецПопытки; 
