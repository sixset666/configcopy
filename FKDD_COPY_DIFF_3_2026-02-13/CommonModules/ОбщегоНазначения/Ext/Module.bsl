...
Функция ПодсистемаСуществует(ПолноеИмяПодсистемы) Экспорт
	...
	ИменаПодсистем = ОбщегоНазначенияПовтИсп.ИменаПодсистем();
...
Функция СократитьСтрокуКонтрольнойСуммой(Строка, МаксимальнаяДлина) Экспорт
	ОбщегоНазначенияКлиентСервер.Проверить(МаксимальнаяДлина >= 32, НСтр(
...
Функция ЕстьПравоНаИспользованиеИнструментов() Экспорт
	Возврат ПравоДоступа("Просмотр", Метаданные.Подсистемы.УниверсальныеИнструменты);
КонецФункции
	...
	Для Каждого ОбъектМетаданных Из Коллекция Цикл
		...
		Для Каждого КлючЗначение Из КоллекцииРеквизитов Цикл
			...
			Для Каждого ТекРеквизит Из КлючЗначение.Значение Цикл
				ОписаниеКоллекцииРеквизитов.Вставить(ТекРеквизит.Имя, ОписаниеРеквизитаОбъектаМетаданных(ТекРеквизит, ТипВсеСсылки));
			КонецЦикла;
			...
			Для Каждого ТЧ ИЗ КлючЗначение.Значение Цикл
				...
				Для Каждого ТекКоллекцияРеквизитовТЧ ИЗ КоллекцииРеквизитовТЧ Цикл
					...
					Для Каждого ТекРеквизит ИЗ ТекКоллекцияРеквизитовТЧ.Значение Цикл
						ОписаниеКоллекцииРеквизитовТЧ.Вставить(ТекРеквизит.Имя, ОписаниеРеквизитаОбъектаМетаданных(ТекРеквизит, ТипВсеСсылки));
					КонецЦикла;
...
Процедура ДобавитьВКоманднуюПанельОбщиеКоманды(Форма, ОсновнаяКоманднаяПанельФормы)
	...
	ОписаниеКоманды = РаботаСФормами.НовыйОписаниеКомандыКнопки();
	ОписаниеКоманды.Имя = "ОткрытьНовуюФормуИнструмента";
	...
	РаботаСФормами.СоздатьКомандуПоОписанию(Форма, ОписаниеКоманды);
	РаботаСФормами.СоздатьКнопкуПоОписанию(Форма, ОписаниеКоманды);
КонецПроцедуры
...
Функция ВариантРедактораКода1С() Экспорт
...
Функция ОписаниеТипаВсеСсылки() Экспорт
	...
	Возврат ОбщегоНазначенияПовтИсп.ОписаниеТипаВсеСсылки();
...
Процедура ДобавитьМассивОбъектовКСравнению(Объекты) Экспорт
	...
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
		ОбщегоНазначенияКлиентСервер.КлючОбъектаВХранилищеНастроек(), КлючНастроекОбъектовКСравнению(),
		МассивОбъектовКСравнению);

КонецПроцедуры

Функция ОбъектыДобавленныеКСравнению() Экспорт
	КлючОбъекта=ОбщегоНазначенияКлиентСервер.КлючОбъектаВХранилищеНастроек();
...
Процедура ОчиститьОбъектыДобавленныеКСравнению() Экспорт
	ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(
		ОбщегоНазначенияКлиентСервер.КлючОбъектаВХранилищеНастроек(), КлючНастроекОбъектовКСравнению(), Новый Массив);
КонецПроцедуры
...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач Параметры = Неопределено) Экспорт
	...
	ПараметрыЗаписи=ОбщегоНазначенияКлиентСервер.СтруктураПараметровЗаписиПоУмолчанию();
	...
	ЗначениеПараметра = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПараметрыЗаписи");
	...
	Попытка
		...
		Попытка
			Если ПараметрыВыполнения.ПараметрыЗаписи.ЗаписьВРежимеЗагрузки Тогда
				...
				НовыеПараметрыВыполнения = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПараметрыВыполнения);
...
Процедура ЗаписатьОбъектПриЗаменеСсылок(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если ЭтоПоследовательность(МетаданныеОбъекта) И Не ПравоДоступа("Изменение", МетаданныеОбъекта)
		И ПользователиМотивации.ЭтоПолноправныйПользователь( , , Ложь) Тогда
...
Функция ПолучитьСсылкуСправочникАлгоритмы(Алгоритм) Экспорт
	Если ТипЗнч(Алгоритм) = Тип("СправочникСсылка.Алгоритмы") Тогда
		Возврат Алгоритм;
	ИначеЕсли ТипЗнч(Алгоритм) = Тип("УникальныйИдентификатор") Тогда
		Возврат Справочники.Алгоритмы.ПолучитьСсылку(Алгоритм);
	ИначеЕсли ТипЗнч(Алгоритм) = Тип("Строка") Тогда
		Если Лев(Алгоритм, 5) = "GUID_" Тогда // БСП внеш. обработка
			СтрокаУИД = Сред(Алгоритм, 6);
			ref = Справочники.Алгоритмы.ПолучитьСсылку(Новый УникальныйИдентификатор(СтрокаУИД));
			Возврат ?(ПустаяСтрока(ref.Наименование), Неопределено, ref);
		КонецЕсли;
		НайденПоНаименованию = Справочники.Алгоритмы.НайтиПоНаименованию(Алгоритм, Истина);
		Если НайденПоНаименованию = Неопределено Тогда
			Попытка
				ЧислоКод = Число(Прав(Алгоритм, 5));
				НайденПоКоду = Справочники.Алгоритмы.НайтиПоКоду(ЧислоКод);
...
Функция ВыполнитьПроцедуруПередЗаписьюОбъекта(Объект, _ТекстПроцедурыПередЗаписью)
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон("Объект: %1. Ошибка при выполнении процедуры ПередЗаписью:
