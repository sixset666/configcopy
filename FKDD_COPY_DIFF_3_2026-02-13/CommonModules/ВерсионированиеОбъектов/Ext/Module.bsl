...
Функция ЗначениеСсылочногоТипа(Значение) Экспорт
	...
	Возврат Истина;//ЭтоСсылка(ТипЗнч(Значение));
...
Процедура ЗаписатьВерсиюОбъекта(Знач Источник, РежимЗаписи = Неопределено) Экспорт
	...
	Если Стрнайти(Строка(ТипЗнч(Источник) ), "объект") = 0
		и Стрнайти(Строка(ТипЗнч(Источник) ), "Регистр сведений") = 0 Тогда
...
Процедура СоздатьВерсиюОбъектаПоОбменуДанными(Объект, СведенияОВерсииОбъекта, СсылкаСуществует, Отправитель) Экспорт
	...
	Если Не ЗначениеЗаполнено(СведенияОВерсииОбъекта.АвторВерсии) Тогда
		СведенияОВерсииОбъекта.АвторВерсии = ПараметрыСеанса.Пользователь;
	КонецЕсли;
...
Функция ЕстьПравоЧтенияДанныхВерсийОбъектов() Экспорт
	Возврат Истина;//ПравоДоступа("Просмотр", Метаданные.ОбщиеКоманды.ИсторияИзменений);
КонецФункции
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	//Если 
	//	Не ПолучитьФункциональнуюОпцию("ИспользоватьВерсионированиеОбъектов")
...
Процедура СоздатьВерсиюОбъекта(Объект, СведенияОВерсииОбъекта, ЗаписьОбычнойВерсии = Истина)
	...
	Если ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.КонтактнаяИнформация")
		или ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.Автомобили")
		или ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.КонтактныеЛицаКонтрагентов")Тогда
		
		ОбъектСсылка = ОБъект.Отбор[0].Значение;
	...
	Если ЗаписьОбычнойВерсии Тогда
		...
		Если (Стрнайти(Строка(ТипЗнч(Объект) ), "Регистр сведений") или Не Объект.ЭтоНовый()) И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(ОбъектСсылка) Тогда
					...
					СоздатьВерсиюОбъекта(ОбъектСсылка.ПолучитьОбъект(), ПараметрыВерсии);
			...
			МенеджерЗаписи.НомерВерсии = НомерПредыдущейВерсии(ОбъектСсылка, СведенияОВерсииОбъекта.НомерВерсии);
			МенеджерЗаписи.Прочитать();
			Если МенеджерЗаписи.Выбран() И Не МенеджерЗаписи.ЕстьДанныеВерсии Тогда
				МенеджерЗаписи.ВерсияОбъекта = Новый ХранилищеЗначения(ДанныеДляХранения(ОбъектСсылка), Новый СжатиеДанных(9));
				МенеджерЗаписи.Записать();
			КонецЕсли;
		КонецЕсли;
		
		СсылкаНаОбъект = ОбъектСсылка;;
		Если СсылкаНаОбъект.Пустая() Тогда
			СсылкаНаОбъект = Объект.ПолучитьСсылкуНового();
			Если СсылкаНаОбъект.Пустая() Тогда
				СсылкаНаОбъект = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ОбъектСсылка).ПолучитьСсылку();
		...
		Если Не Объект.ДополнительныеСвойства.Свойство("АвторВерсии", АвторВерсии) Тогда
			АвторВерсии = ПараметрыСеанса.Пользователь;
		КонецЕсли;
		...
		Если (Стрнайти(Строка(ТипЗнч(Объект) ), "Регистр сведений") или Не Объект.ЭтоНовый())  Тогда
...
Процедура ПриСозданииВерсииОбъекта(Объект, РежимЗаписи)
	...
	НомерПоследнейВерсии = НомерПоследнейВерсии(ОбъектСсылка);
...
Функция ВерсияОтличаетсяОтРанееЗаписанной(Объект)
	...
	Возврат ТипРС или Объект.ЭтоНовый() Или КонтрольнаяСумма(ДанныеДляХранения(Объект)) <> КонтрольнаяСумма(ДанныеДляХранения(Объект.Ссылка.ПолучитьОбъект()));
...
Процедура ДобавитьКомментарийКВерсии(СсылкаНаОбъект, НомерВерсии, Комментарий) Экспорт
	...
	Если МенеджерЗаписи.Выбран() Тогда
		Если МенеджерЗаписи.АвторВерсии = ПараметрыСеанса.Пользователь Тогда
...
Процедура ВывестиШапкуПоВерсии(Результат, Знач Текст, Знач НомерСтроки, Знач НомерКолонки)
	...
	Если Не ПустаяСтрока(Текст) Тогда
		...
		Результат.Область(Регион).ЦветФона = ЦветаСтиля.ЦветОсобогоТекста;
		Результат.Область(Регион).Шрифт = ШрифтыСтиля.КрупныйШрифтТекста;
...
Функция ВывестиРеквизитыПоРазобранномуОбъекту(Результат, ВерсияОбъекта, СсылкаНаОбъект)
	...
	Для Каждого ЭлементРеквизит Из Реквизиты Цикл
		...
		УстановитьСвойстваТекста(Секция.Область("R1C2"), ВыводимоеНаименование, ШрифтыСтиля.КрупныйШрифтТекста);
		...
		Секция.Область("R1C2:R1C3").ЦветРамки = ЦветаСтиля.ЦветВажного;
...
Процедура ВывестиТабличныеЧастиПоРазобранномуОбъекту(Результат, ВерсияОбъекта, НомерСтрокиВывода, СсылкаНаОбъект)
	...
	Для Каждого СтрокаТабличнаяЧасть Из ВерсияОбъекта.ТабличныеЧасти Цикл
		...
		УстановитьСвойстваТекста(ОбластьСекции, "N", ШрифтыСтиля.КрупныйШрифтТекста, Истина);
		ОбластьСекции.ЦветФона = ЦветаСтиля.ЦветВажного;
		...
		Для Каждого КолонкаТабличнойЧасти Из ТабличнаяЧасть.Колонки Цикл
			...
			ЦветЗаголовкаКолонки = ?(ОписаниеПоля = Неопределено, ЦветаСтиля.ТекстПредупреждающейНадписи, ЦветаСтиля.ЦветВажного);
			СекцияОбласти = Секция.Область("R1C1");
			УстановитьСвойстваТекста(СекцияОбласти, ВыводимоеНаименованиеПоля, ШрифтыСтиля.КрупныйШрифтТекста, Истина);
...
Процедура ВывестиТабличныеДокументыПоРазобранномуОбъекту(ТабличныйДокумент, ОписаниеОбъекта)
	...
	Для Каждого ЭлементСтруктуры Из ТабличныеДокументы Цикл
		НаименованиеТабличногоДокумента = ЭлементСтруктуры.Наименование;
		...
		ВыводимыйДокумент = ЭлементСтруктуры.Данные;
...
Функция ПолучитьДанныеПоАвтомобилям(СсылкаНаОбъект)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Автомобили.Значение = &Значение";
	
	Запрос.УстановитьПараметр("Значение", СсылкаНаОбъект);
...
Функция ПолучитьДанныеПоКонтактныеЛицаКонтрагентов(СсылкаНаОбъект)
	...
	Запрос.УстановитьПараметр("Ведущий", СсылкаНаОбъект);
...
Функция ПолучитьДанныеПоКонтактнаяИнформация(СсылкаНаОбъект)
	...
	Запрос.Текст = 
		...
		|	КонтактнаяИнформация.КраткоеПредставление КАК КраткоеПредставление
		|ИЗ
...
Функция ПолучитьДанныеПоДоговорыВзаиморасчетов(СсылкаНаОбъект)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ *
		|ИЗ
		|	Справочник.ДоговорыВзаиморасчетов КАК КонтактнаяИнформация
		|ГДЕ
		|	КонтактнаяИнформация.Владелец = &Объект";
...
Функция ТабличныеДокументыОбъекта(СсылкаНаОбъект)
	
	МассивВозврата = новый Массив;
	
	ТаблицаДанных = ПолучитьДанныеПоАвтомобилям(СсылкаНаОбъект); 
	
	Если ТаблицаДанных.Количество() <> 0 Тогда
		
		ТабДок = ПолучитьТабличныйДокумент(ТаблицаДанных);
		
		МассивВозврата.Добавить(новый Структура("Наименование, Данные", "Автомобили", ТабДок)); 
		
	КонецЕсли;                                       
	
	ТаблицаДанных = ПолучитьДанныеПоКонтактнаяИнформация(СсылкаНаОбъект);     
	
	Если ТаблицаДанных.Количество() <> 0 Тогда
		
		ТабДок = ПолучитьТабличныйДокумент(ТаблицаДанных);
		
		МассивВозврата.Добавить(новый Структура("Наименование, Данные", "Контактная информация", ТабДок));      
		
	КонецЕсли;
	
	ТаблицаДанных = ПолучитьДанныеПоКонтактныеЛицаКонтрагентов(СсылкаНаОбъект);  
	
	Если ТаблицаДанных.Количество() <> 0 Тогда
		
		ТабДок = ПолучитьТабличныйДокумент(ТаблицаДанных);
		
		МассивВозврата.Добавить(новый Структура("Наименование, Данные", "Контактные лица", ТабДок));
	КонецЕсли;     
	
	ТаблицаДанных = ПолучитьДанныеПоДоговорыВзаиморасчетов(СсылкаНаОбъект);  
	
	Если ТаблицаДанных.Количество() <> 0 Тогда
		
		ТабДок = ПолучитьТабличныйДокумент(ТаблицаДанных);
		
		МассивВозврата.Добавить(новый Структура("Наименование, Данные", "Договоры взаиморасчетов", ТабДок));
	КонецЕсли;
...
Функция ДанныеДляХранения(Знач Объект)
	...
	Если Стрнайти(Строка(ТипЗнч(Объект) ), "объект") = 0 
		и Стрнайти(Строка(ТипЗнч(Объект) ), "Регистр сведений") = 0 Тогда 
		
		Объект = Объект.ПолучитьОбъект();                               
		
	ИначеЕсли Не Стрнайти(Строка(ТипЗнч(Объект) ), "Регистр сведений") = 0 Тогда 
		
		СсылкаНаОбъект = ОБъект.Отбор[0].Значение;
	...
	Если Ложь Тогда //ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		//МодульУправлениеКонтактнойИнформациейСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформациейСлужебный");
		//МодульУправлениеКонтактнойИнформациейСлужебный.
		ПриПодготовкеДанныхОбъекта(Объект, ДополнительныеРеквизиты);
