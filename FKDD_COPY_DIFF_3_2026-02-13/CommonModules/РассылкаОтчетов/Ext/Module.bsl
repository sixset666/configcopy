...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	ТипНастроекОтчета = Новый ОписаниеТипов(МассивТиповНастроек);
	
	// Таблица отчетов
	ТаблицаОтчетов = Новый ТаблицаЗначений;
	ТаблицаОтчетов.Колонки.Добавить("Отчет",                  Новый ОписаниеТипов("СправочникСсылка.ВариантыОтчетов"));
	ТаблицаОтчетов.Колонки.Добавить("ОтправлятьЕслиПустой",   Новый ОписаниеТипов("Булево"));
	ТаблицаОтчетов.Колонки.Добавить("Настройки",              ТипНастроекОтчета);
	...
	ТаблицаОтчетов.Колонки.Добавить("ВариантЗагрузкиПериода", Новый ОписаниеТипов("Строка"));
	
	// Форматы по умолчанию
	ФорматыПоУмолчанию = Новый Массив;
	Найденные = Рассылка.ФорматыОтчетов.НайтиСтроки(Новый Структура("Отчет", Справочники.ВариантыОтчетов.ПустаяСсылка()));
	...
	Для Каждого СтрокаОтчет Из Рассылка.Отчеты Цикл
		...
		Стр.ВариантЗагрузкиПериода = СтрокаОтчет.ВариантЗагрузкиПериода;
	...
	ПараметрыДоставки.Вставить("Автор",                        ПараметрыСеанса.Пользователь);
	...
	Если НЕ ПараметрыДоставки.ИспользоватьСетевойКаталог
...
// Формирует отчеты и отправляет согласно настройкам транспорта (FILE, EMAIL, FTP);
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Если НЕ ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ПараметрыДоставки, Рассылка, ПараметрыЖурнала) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		...
		ПараметрыОтчета = Новый Структура("Отчет, Настройки, Форматы, ОтправлятьЕслиПустой, ВариантЗагрузкиПериода");
		...
		Если НЕ ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	|	РассылкиОтчетов.Подготовлена КАК Подготовлена,
	|	ЛОЖЬ КАК Выполнена,
	|	ЛОЖЬ КАК СОшибками
	|ИЗ
	|	Справочник.РассылкиОтчетов КАК РассылкиОтчетов
	|ГДЕ
	|	РассылкиОтчетов.Ссылка В ИЕРАРХИИ(&МассивРассылок) И 
	|	РассылкиОтчетов.ЭтоГруппа = ЛОЖЬ И 
	...
	ИначеЕсли Всего <= 5 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			СтрПредставление = "'"+ СтрокаТаблицы.Представление +"'";
			Если НЕ СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("ru = 'Рассылка "+СтрПредставление+" не подготовлена'");
			ИначеЕсли НЕ СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("ru = 'Рассылка "+СтрПредставление+" не выполнена'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("ru = 'Рассылка "+СтрПредставление+" выполнена с ошибками'");
			Иначе
				ШаблонСообщения = НСтр("ru = 'Рассылка "+СтрПредставление+" выполнена'");
			КонецЕсли;
			
			Результат.Текст = Результат.Текст + ?(Результат.Текст= "", "", Символы.ПС + Символы.ПС) + ШаблонСообщения;
			
		КонецЦикла;
	Иначе
		Стр1 = Формат(Подготовлено, "ЧН=0; ЧГ=0");
		Стр2 = Формат(Всего, "ЧН=0; ЧГ=0");
		Стр3 = Формат(Выполнено,    "ЧН=0; ЧГ=0");
		Стр4 = Формат(СОшибками,    "ЧН=0; ЧГ=0");
		Стр5 = Формат(НеВыполнено,  "ЧН=0; ЧГ=0");
		Результат.Текст = НСтр("ru = 'Подготовлено рассылок: "+Стр1+" из "+Стр2+"
			|Выполнено: "+Стр3+"
			|С ошибками: "+Стр4+"
			|Не выполнено: "+Стр5+"'");
	КонецЕсли;
...
//
Процедура ВыполнитьРассылкуПоРасписанию(ПараметрыРассылки) Экспорт
	
	Если ПустаяСтрока(ИмяПользователя()) Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
	
	ОшибкаЗаполненияРассылки = Ложь;
	ОшибкаЗаполненияРассылки = ОшибкаЗаполненияРассылки ИЛИ (НЕ ТипЗнч(ПараметрыРассылки) = Тип("Структура"));
	ОшибкаЗаполненияРассылки = ОшибкаЗаполненияРассылки ИЛИ (НЕ ПараметрыРассылки.Свойство("Рассылка"));
	
	Если ОшибкаЗаполненияРассылки Тогда
		ВызватьИсключение НСтр("ru = 'Не задан элемент рассылки отчетов'");
	КонецЕсли;
	
	Рассылка = ПараметрыРассылки.Рассылка;
	
	// Проверки
	Если НЕ Рассылка.ВыполнятьПоРасписанию Тогда
		ВызватьИсключение НСтр("ru = 'У рассылки отчетов """+Строка(Рассылка)+""" отключен флажок ""Выполнять по расписанию""'");
	КонецЕсли;
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	МассивГруппПолучателей = Новый Массив();
	Если Рассылка.Личная Тогда
		...
		МассивПолучателей.Добавить(Рассылка.Автор);
		
	Иначе
		
		ТипПолучателей = Рассылка.ТипПолучателейРассылки;
		Если ТипПолучателей = "Пользователи" Тогда
			ТипПолучателей = Тип("СправочникСсылка.Пользователи");
		Иначе
			ТипПолучателей = Тип("СправочникСсылка.Контрагенты");
		КонецЕсли;
		МассивПолучателей = Новый Массив();
		//ТаблицаПолучателей = Рассылка.Получатели.Выгрузить();
		Если ТипЗнч(Рассылка.Получатели) = Тип("Массив") Тогда
			Для Каждого Получатель Из Рассылка.Получатели Цикл
				Если Получатель.ЭтоГруппа Тогда
					МассивГруппПолучателей.Добавить(Получатель);
				Иначе
					МассивПолучателей.Добавить(Получатель);
				КонецЕсли;
			КонецЦикла;
		Иначе
			Для Каждого ТекСтрока Из Рассылка.Получатели Цикл
				Если НЕ ТекСтрока.Исключен Тогда
					Если ТекСтрока.Получатель.ЭтоГруппа Тогда
						МассивГруппПолучателей.Добавить(ТекСтрока.Получатель);
					Иначе
						МассивПолучателей.Добавить(ТекСтрока.Получатель);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	ВидИерархии = 0;
	ОтборПоГруппам = "";
	Если ПолучателиМД.Иерархический Тогда
		ВидИерархии = 1;
		ОтборПоГруппам = " И 
		|	Получатели.ЭтоГруппа = Ложь";
	...
	Если ЗначениеЗаполнено(ВидКИ) Тогда
		ОтборПоВидуКИ = " И 
		|	тчКонтактнаяИнформация.Вид = &ВидКИ";
	КонецЕсли;
	
	Если ТипПолучателей = Тип("СправочникСсылка.Пользователи") Тогда
		ТекстПоле = "Получатели.Сотрудник";
	Иначе
		ТекстПоле = "Получатели.Ссылка";
	КонецЕсли;
	
	ИмяОбъекта = ПолучателиМД.ПолноеИмя();
	...
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	" + ТекстПоле + " КАК Получатель
	|ПОМЕСТИТЬ
	|	втПолучатели
	|ИЗ
	|	" + ИмяОбъекта + " КАК Получатели
	|ГДЕ
	|	Получатели.Ссылка В (&МассивПолучателей) И 
	|	Получатели.ПометкаУдаления = ЛОЖЬ" + ОтборПоГруппам + ?(ВидИерархии = 0, "", "
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	" + ИмяОбъекта + " КАК Получатели
	|ГДЕ
	|	Получатели.Ссылка В ИЕРАРХИИ (&МассивГруппПолучателей) И 
	|	Получатели.ПометкаУдаления = ЛОЖЬ") + ОтборПоГруппам + "
	|;
	|
	|ВЫБРАТЬ
	...
	|ПО
	|	втПолучатели.Получатель = тчКонтактнаяИнформация.Объект И 
	|	тчКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)" + ОтборПоВидуКИ + "
	|ГДЕ
	|	тчКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты)" + ОтборПоВидуКИ + "
	|";
		
	Запрос.УстановитьПараметр("МассивПолучателей",      МассивПолучателей);
	Запрос.УстановитьПараметр("МассивГруппПолучателей", МассивГруппПолучателей);
...
//
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	МассивРеквизитов = обРазложитьСтрокуВМассивПодстрок(
	...
	Если ПараметрыОтчета.ЭтоВариант Тогда
		...
		ИмяОтчета = ПараметрыОтчета.Отчет.ИмяОтчета;
		ПараметрыОтчета.Имя        = ИмяОтчета;
		ПараметрыОтчета.Метаданные = Метаданные.Отчеты.Найти(ИмяОтчета);
		...
		ПараметрыОтчета.Объект = Отчеты[ИмяОтчета].Создать();
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		Попытка
			ПараметрыОтчета.Объект.ЗаполнитьНачальныеНастройки();
		Исключение
			ОтчетыСервер.ЗаполнитьНачальныеОстатки(ПараметрыОтчета.Объект);
		КонецПопытки;
		...
		Если ТипЗнч(ПараметрыОтчета.Настройки) = Тип("НастройкиКомпоновкиДанных") Тогда
			ПараметрыОтчета.НастройкиКД = ПараметрыОтчета.Настройки;
		ИначеЕсли ПараметрыОтчета.ЭтоВариант Тогда
		...
		ОтчетыСервер.УстановитьПериодСКДПоВарианту(КомпоновщикНастроекКД.Настройки, ПараметрыОтчета.ВариантЗагрузкиПериода);
		
		// Персональные отборы
		//Если ПерсонализацияВозможна Тогда
			
			//ПользовательскиеНастройкиКД = КомпоновщикНастроекКД.ПользовательскиеНастройки;
			//КомпоновщикНастроекКД.Настройки.Отбор;
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено) Экспорт
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		Если Получатель <> Неопределено И ПараметрыОтчета.Свойство("ПолеОтбора") Тогда
			...
			ДоступноеПолеОтбора = КомпоновщикНастроекКД.Настройки.Отбор.ДоступныеПоляОтбора.НайтиПоле(ПолеОтбора);
			Если ДоступноеПолеОтбора = Неопределено ИЛИ (НЕ ДоступноеПолеОтбора.ТипЗначения.СодержитТип(ТипЗнч(Получатель))) Тогда
				Возврат Результат;
			Иначе
				Отбор = КомпоновщикНастроекКД.Настройки.Отбор.Элементы;
		...
		Заголовок = ПараметрыОтчета.Отчет.ПредставлениеОтчета;
		ОтчетыСервер.СформироватьОтчет(ОтчетОбъект, Результат.ТабДок, , КомпоновщикНастроекКД, Заголовок);
...
Процедура ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурнала = Неопределено, КомментарийСПараметрами = "", 
	...
	Если Параметр1 <> Неопределено Тогда
		КомментарийСПараметрами = СтрЗаменить(КомментарийСПараметрами, "%1", Параметр1);
		КомментарийСПараметрами = СтрЗаменить(КомментарийСПараметрами, "%2", Параметр2);
		КомментарийСПараметрами = СтрЗаменить(КомментарийСПараметрами, "%3", Параметр3);
		КомментарийСПараметрами = СтрЗаменить(КомментарийСПараметрами, "%4", Параметр4);
	КонецЕсли;
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ТекстДляПользователя = ТекстДляПользователя + Символы.ПС + ОписаниеОшибки;
	ИначеЕсли ТипЗнч(ИнформацияОбОшибке) = Тип("Строка") Тогда
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого ТекФормат Из ПараметрыОтчета.Форматы Цикл
		
		ПараметрыФормата = ПараметрыДоставки.ПараметрыФорматов.Получить(ТекФормат);
		...
		СтрокаФайл.Настройки.Формат = ТекФормат;
...
//
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		...
		Иначе
			...
			ПараметрыДоставки.СетевойКаталогWindows = ДобавитьКонечныйРазделительПути(
				ПараметрыДоставки.СетевойКаталогWindows
			);
			ПараметрыДоставки.СетевойКаталогLinux = ДобавитьКонечныйРазделительПути(
	...
	Для Каждого ФорматМД Из Метаданные.Перечисления.ФорматыСохраненияОтчетов.ЗначенияПеречисления Цикл
		ТекФормат = Перечисления.ФорматыСохраненияОтчетов[ФорматМД.Имя];
		ПараметрыФормата = ПараметрыЗаписиТабличногоДокументаВФормат(ТекФормат);
		ПараметрыФормата.Вставить("Имя", ФорматМД.Имя);
		ПараметрыДоставки.ПараметрыФорматов.Вставить(ТекФормат, ПараметрыФормата);
	КонецЦикла;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту 
		И (ПараметрыДоставки.ИспользоватьСетевойКаталог
			ИЛИ ПараметрыДоставки.ИспользоватьFTPРесурс)
...
Процедура ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель = Неопределено)
	...
	//зфОтправитьПисьмаЧерезСтандартныйSMTP(УчетнаяЗапись, Письмо, СоединениеДляОтправки=Неопределено, мсвОшибки=Неопределено)
	...
	//ПараметрыПисьма.Вставить("ФорматТекста", ПараметрыДоставки.ФорматТекста);
	...
	Если Получатель = Неопределено Тогда
		...
		Если ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеТемы ИЛИ ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеСообщения Тогда
			...
			Для Каждого КлючИЗначение Из ПараметрыДоставки.Получатели Цикл
				...
				// Отправляем письмо
				ОтправитьПочтовоеСообщение(ПараметрыДоставки.УчетнаяЗапись, ПараметрыПисьма, ПараметрыДоставки.Соединение);
			КонецЦикла;
		Иначе
			...
			ПараметрыПисьма.Вставить("Получатели", ПараметрыДоставки.Получатели);
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если НЕ ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			...
			"'"+ Строка(ПараметрыСеанса.Пользователь) +"'"
		);
	КонецЕсли;
	Возврат ПравоВывода;
КонецФункции
...
Процедура ПриОпределенииНастроекПрокси(Прокси) Экспорт
	...
	//Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		...
		//Модуль.ПриОпределенииНастроекПрокси(Прокси);
	//КонецЕсли;
...
//		Возврат Истина;
//	ИначеЕсли ТипЗнч(Вложение) = Тип("Строка") Тогда
//		Попытка
//			ТекФайл = Новый Файл(Вложение);
//			Если ТекФайл.Существует() Тогда
//				ПолучитьИмяВременногоФайла(Вложение)
//				Новый Файл
//			КонецЕсли;
//		Исключение
//			Возврат Ложь;
...
Функция ПривестиВложениеДляВставкиВПисьмо(Вложение) Экспорт
	...
	ИначеЕсли ТипЗнч(Вложение) = Тип("Строка") Тогда
		Попытка
			Вложение = Новый Файл(Вложение);
			Возврат ПривестиВложениеДляВставкиВПисьмо(Вложение);
		Исключение
			Возврат Ложь;
		КонецПопытки;
	КонецЕсли;
...
Функция ОтправитьПочтовоеСообщение(знач УчетнаяЗапись,
	...
	//НовоеПисьмо.Автор                    = ПараметрыСеанса.Пользователь;
	//НовоеПисьмо.ПодразделениеКомпании    = ПараметрыСеанса.ПодразделениеКомпании;
	//НовоеПисьмо.Организация              = ПараметрыСеанса.Организация;
	//НовоеПисьмо.ХозОперация              = Справочники.ХозОперации.ЭлектронноеПисьмо;
	//НовоеПисьмо.УчетнаяЗапись            = УчетнаяЗапись;
	//НовоеПисьмо.ТекстПисьма              = ПараметрыПисьма.Тело;
	//НовоеПисьмо.Тема                     = ПараметрыПисьма.Тема;
	//НовоеПисьмо.ПолучателиПредставление  = ПараметрыПисьма.Кому;
	//НовоеПисьмо.ГруппаПисем              = Справочники.ГруппыПисемЭлектроннойПочты.НайтиПоНаименованию("Исходящие",Истина,,НовоеПисьмо.УчетнаяЗапись);
	//НовоеПисьмо.ОтправительПредставление = ?(СокрЛП(НовоеПисьмо.УчетнаяЗапись.Пользователь) = "", "<", НовоеПисьмо.УчетнаяЗапись.Пользователь + " <") + НовоеПисьмо.УчетнаяЗапись.АдресЭлектроннойПочты + ">" ;
	//
	//Для Каждого ТекПолучатель Из ПараметрыПисьма.Получатели Цикл
	//	Получатель = НовоеПисьмо.Получатели.Добавить();
	//	Получатель.КодГруппыАдреса       = Перечисления.КодГруппыАдресаЭлектронногоПисьма.Кому;
	//	Получатель.АдресЭлектроннойПочты = ТекПолучатель.Значение;
	//	Получатель.Представление         = СокрЛП(ТекПолучатель.Ключ);
	//	Если ТипЗнч(ТекПолучатель.Ключ) = Тип("СправочникСсылка.Контрагенты") Тогда
	//		Получатель.СсылкаНаОбъект    = ТекПолучатель.Ключ;
	//	КонецЕсли;
	//КонецЦикла;
	//
	...
	//			НоваяСтрока = НовоеПисьмо.ПрикрепленныеФайлы.Добавить();
	//			НоваяСтрока.Имя    = КлючИЗначение.Ключ;
	//			НоваяСтрока.Файл   = Новый ХранилищеЗначения(Вложение);
	//			Попытка
...
Функция РазобратьСтрокуСПочтовымиАдресами(знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	МассивПодстрок           = обРазложитьСтрокуВМассивПодстрок(СтрокаПереданныхАдресов, ";");
	МассивПодстрокКОбработке = Новый Массив;
	
	Для каждого ЭлементМассива Из МассивПодстрок Цикл
		Если Найти(ЭлементМассива,",") > 0 Тогда
			ДопМассивПодстрок = обРазложитьСтрокуВМассивПодстрок(СтрокаПереданныхАдресов);
	...
	Для Каждого СтрокаАдреса Из МассивПодстрокКОбработке Цикл
				...
				Если      СтадияРазбора = 1 Тогда
					...
					Для ИндексПоискаНС = 1 По СтрДлина(Накопитель) Цикл
						Если Найти(НедопустимыеСимволы, Сред(Накопитель, ИндексПоискаНС, 1)) > 0 И ВызыватьИсключение Тогда
							ВызватьИсключение НСтр("ru = 'Недопустимый символ "+Сред(Накопитель, ИндексПоискаНС, 1)+" в адресе электронной почты "+СтрокаАдреса+"'");
						КонецЕсли;
					КонецЦикла;
					
					Накопитель = Накопитель + Символ;
				ИначеЕсли СтадияРазбора = 2 И ВызыватьИсключение Тогда
					ВызватьИсключение НСтр("ru = 'Некорректный адрес электронной почты "+СтрокаАдреса);
			...
			Иначе
				Если СтадияРазбора = 2 ИЛИ СтадияРазбора = 3 Тогда
					Если Найти(НедопустимыеСимволы, Символ) > 0 И ВызыватьИсключение Тогда
						ВызватьИсключение НСтр("ru = 'Недопустимый символ "+Символ+" в адресе электронной почты "+СтрокаАдреса+"'");
					КонецЕсли;
				КонецЕсли;
		...
		Если ПустаяСтрока(ПочтовыйАдрес) И (Не ПустаяСтрока(ПолноеИмяАдресата)) И ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("ru = 'Некорректный адрес электронной почты "+ПолноеИмяАдресата);
		ИначеЕсли СтрЧислоВхождений(ПочтовыйАдрес,"@") <> 1 И ВызыватьИсключение Тогда 
			ВызватьИсключение НСтр("ru = 'Некорректный адрес электронной почты "+ПочтовыйАдрес);
		КонецЕсли;
