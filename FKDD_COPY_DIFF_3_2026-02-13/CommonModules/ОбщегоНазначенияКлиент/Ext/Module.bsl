...
Процедура ПриНачалеРаботыСистемы() Экспорт
	ПараметрыСтартаСеанса=ОбщегоНазначенияВызовСервера.ПараметрыСтартаСеанса();

	Если ПараметрыСтартаСеанса.ДобавленыПраваНаРасширение Тогда
		ЗавершитьРаботуСистемы(Ложь, Истина);
	КонецЕсли;
	ПараметрыПриложения.Вставить("НомерСеанса", ПараметрыСтартаСеанса.НомерСеанса);

	ПараметрыПриложения.Вставить("ЭтоLinuxКлиент", ОбщегоНазначенияКлиентСервер.ЭтоLinux());
	ПараметрыПриложения.Вставить("ЭтоПортативнаяПоставка", ОбщегоНазначенияКлиентСервер.ЭтоПортативнаяПоставка());
	ПараметрыПриложения.Вставить("ПолеHTMLПостроеноНаWebkit",
		ОбщегоНазначенияКлиентСервер.ПолеHTMLПостроеноНаWebkit());
	ПараметрыПриложения.Вставить("ВерсияПлатформы",
		ОбщегоНазначенияКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие());
	//ПараметрыПриложения.Вставить("АдресОписанияМетаданныхКонфигурации", ОбщегоНазначенияВызовСервера.АдресОписанияМетаданныхКонфигурации());
	
	ПараметрыСеансаВХранилище = Новый Структура;
	ПараметрыСеансаВХранилище.Вставить("ЭтоLinuxКлиент", ПараметрыПриложения["ЭтоLinuxКлиент"]);
	ПараметрыСеансаВХранилище.Вставить("ЭтоПортативнаяПоставка", ПараметрыПриложения["ЭтоПортативнаяПоставка"]);
	ПараметрыСеансаВХранилище.Вставить("ПолеHTMLПостроеноНаWebkit", ПараметрыПриложения["ПолеHTMLПостроеноНаWebkit"]);
	ПараметрыСеансаВХранилище.Вставить("ВерсияПлатформы", ПараметрыПриложения["ВерсияПлатформы"]);
	//ПараметрыСеансаВХранилище.Вставить("АдресОписанияМетаданныхКонфигурации", ПараметрыПриложения["АдресОписанияМетаданныхКонфигурации"]);

	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить(
		ОбщегоНазначенияКлиентСервер.КлючОбъектаВХранилищеНастроек(),
		ОбщегоНазначенияКлиентСервер.КлючНастроекПараметровСеанса(), ПараметрыСеансаВХранилище);
...
Функция ВыполнитьАлгоритм(АлгоритмСсылка, ВходящиеПараметры = Неопределено, ОшибкаВыполнения = Ложь,
	...
	Возврат АлгоритмыКлиентСервер.ВыполнитьАлгоритм(АлгоритмСсылка, ВходящиеПараметры, ОшибкаВыполнения,
		СообщениеОбОшибке)
КонецФункции
...
Процедура ОткрытьКонсольОтладки(ТипОбъектаОтладки, ДанныеДляОтладки, УникальностьФормыКонсоли = Неопределено) Экспорт
	Если ВРег(ТипОбъектаОтладки) = "ЗАПРОС" Тогда
		ИмяФормыКонсоли = "Обработка.КонсольЗапросов.Форма";
	ИначеЕсли ВРег(ТипОбъектаОтладки) = "СХЕМАКОМПОНОВКИДАННЫХ" Тогда
		ИмяФормыКонсоли = "Отчет.КонсольОтчетов.Форма";
	ИначеЕсли ВРег(ТипОбъектаОтладки) = "ОБЪЕКТБАЗЫДАННЫХ" Тогда
		ИмяФормыКонсоли = "Обработка.РедакторРеквизитовОбъекта.Форма";
	ИначеЕсли ВРег(ТипОбъектаОтладки) = "HTTPЗАПРОС" Тогда
		ИмяФормыКонсоли = "Обработка.КонсольHTTPЗапросов.Форма";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗапуститьКонсольОтладкиПоКлючуНастройкиДанныхОтладки(КлючНастройкиОтладки, ИдентификаторФормы = Неопределено) Экспорт
	...
	ДанныеОтладки = ОбщегоНазначенияВызовСервера.СтруктураДанныхОбъектаОтладкиИзСистемногоХранилищаНастроек(
...
Процедура ОткрытьФормуРедактированияТекста(Текст, ОписаниеОповещенияОЗакрытии, Заголовок = "",
	...
	Если РежимОткрытия = Неопределено Тогда
		ОткрытьФорму("ОбщаяФорма.ФормаРедактированияТекста", ПараметрыФормы, , , , , ОписаниеОповещенияОЗакрытии);
	Иначе
		ОткрытьФорму("ОбщаяФорма.ФормаРедактированияТекста", ПараметрыФормы, , , , , ОписаниеОповещенияОЗакрытии,
			РежимОткрытия);
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораЭлементовСпискаЗначений(Список, ОписаниеОповещенияОЗакрытии, Заголовок = "",
	...
	Если РежимОткрытия = Неопределено Тогда
		ОткрытьФорму("ОбщаяФорма.ФормаРедактированияСпискаЗначений", ПараметрыФормы, , , , ,
...
Процедура РедактироватьОбъект(СсылкаНаОбъект) Экспорт
	МассивТиповДоступныхДляРедактирования=ОбщегоНазначенияКлиентПовтИсп.ТипыОбъектовДоступныйДляРедактораОбъектовБазыДанных();
	...
	ОткрытьФорму("Обработка.РедакторРеквизитовОбъекта.Форма", ПараметрыФормы);
КонецПроцедуры
...
Процедура РедактироватьJSON(СтрокаJSON, РежимПросмотра, ОписаниеОповещенияОЗавершении = Неопределено) Экспорт
	...
	Если ОписаниеОповещенияОЗавершении = Неопределено Тогда
		ОткрытьФорму("Обработка.РедакторJSON.Форма", СтруктураПараметров);
	Иначе
		ОткрытьФорму("Обработка.РедакторJSON.Форма", СтруктураПараметров, , , , , ОписаниеОповещенияОЗавершении);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьДинамическийСписок(ИмяОбъектаМетаданных, ОписаниеОповещенияОЗавершении = Неопределено) Экспорт
	...
	Если ОписаниеОповещенияОЗавершении = Неопределено Тогда
		ОткрытьФорму("Обработка.ДинамическийСписок.Форма", СтрукПараметры, , ИмяОбъектаМетаданных);
	Иначе
		ОткрытьФорму("Обработка.ДинамическийСписок.Форма", СтрукПараметры, , ИмяОбъектаМетаданных, , ,
...
Процедура НайтиСсылкиНаОбъект(СсылкаНаОбъект) Экспорт
	...
	ОткрытьФорму("Обработка.ПоискСсылокНаОбъект.Форма", ПараметрыФормы);
...
Процедура ЗапуститьПроверкуОбновленияИнструментов() Экспорт
	ПараметрыФормы = Новый Структура;
	ОткрытьФорму("Обработка.ТехПоддержка.Форма.ОбновлениеИнструментов", ПараметрыФормы);
КонецПроцедуры
...
Процедура Подключаемый_ВыполнитьОбщуюКомандуИнструментов(Форма, Команда) Экспорт
	Если Команда.Имя = "ОткрытьНовуюФормуИнструмента" Тогда
...
Процедура ДобавитьОбъектыКСравнению(МассивОбъектов, Контекст) Экспорт
	ОбщегоНазначенияКлиентСервер.ДобавитьМассивОбъектовКСравнению(МассивОбъектов);
КонецПроцедуры

Процедура ВыгрузитьОбъектыВXML(МассивОбъектов, Контекст) Экспорт
	АдресФайлаВоВременномХранилище="";
	ОбщегоНазначенияВызовСервера.ВыгрузитьОбъектыВXMLНаСервере(МассивОбъектов, АдресФайлаВоВременномХранилище,
...
Процедура ОткрытьНастройкиОтладкиДополнительнойОбработки(СсылкаНаОбъект) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.НастройкиОтладкиДополнительныхОбработок", ПараметрыФормы);
КонецПроцедуры
...
Процедура РедактироватьТип(ТипДанных, РежимЗапуска, СтандартнаяОбработка, ВладелецФормы, ОписаниеОповещенияОЗавершении) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.РедактированиеТипаЗначения", ПараметрыФормы, ВладелецФормы, , , ,
...
Процедура РедактироватьТаблицуЗначений(ТаблицаЗначенийСтрокой, ВладелецФормы, ОписаниеОповещенияОЗавершении) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.РедакторТаблицыЗначений", ПараметрыФормы, ВладелецФормы, , , ,
		ОписаниеОповещенияОЗавершении);
КонецПроцедуры
...
Процедура РедактироватьХранилищеЗначения(Форма, АдресВременногоХранилищаЗначенияИлиЗначение,
	...
	ОткрытьФорму("ОбщаяФорма.ФормаХранилищаЗначения", ПараметрыФормы, Форма, Форма.УникальныйИдентификатор, , ,
...
Процедура РедактироватьПараметрыЗаписи(Форма) Экспорт
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ПараметрыЗаписи", ОбщегоНазначенияКлиентСервер.ПараметрыЗаписиФормы(Форма));
	...
	ОткрытьФорму("ОбщаяФорма.ПараметрыЗаписи", ПараметрыФормы, Форма, , , , ОписаниеОповещенияОЗакрытии,
...
Процедура РедактироватьПараметрыЗаписиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрыЗаписиНаФорму(Форма, Результат);
КонецПроцедуры
...
Функция ДиалогВыбораФайлаПоСтруктуреОписанияВыбираемогоФайла(Режим, СтруктураОписанияВыбираемогоФайла) Экспорт
	...
	Если Не (ОбщегоНазначенияКлиентСервер.ЭтоLinux() И Найти(СтруктураОписанияВыбираемогоФайла.ИмяФайла, "-") > 0) Тогда
...
Процедура СохранитьДанныеКонсолиВФайлНачатьПолучениеФайла(ИмяФайла, ДополнительныеПараметры) Экспорт
	...
	ПодготовленныеДанныеДляЗаписи=ОбщегоНазначенияВызовСервера.ПодготовленныеДанныеКонсолиДляЗаписиВФайл(
...
Процедура СохранитьДанныеКонсолиВФайлПослеПолученияФайлов(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		Если ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.13") Тогда
...
Процедура ПодключитьРасширениеРаботыСФайламиСВозможнойУстановкойЗавершениеПодключенияРасширения(Подключено,
	...
	Если Подключено Тогда
		СтруктураФайловыхПеременныхСеанса=ПараметрыПриложения[ИмяПараметраФайловыхПеременныхСеанса()];
...
Функция НомерСеанса() Экспорт
	Возврат ПараметрыПриложения["НомерСеанса"];
КонецФункции
...
Функция СтруктураФайловыхПеременныхСеанса() Экспорт
	ТекущиеПараметрыПриложения=ПараметрыПриложения;
...
#Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоWindows() Тогда
		...
		ИначеЕсли СоответствиеПоказателейСтрокиСоединения.Свойство("srvr") Тогда
			ПутьКБазе = СтроковыеФункцииКлиентСервер.ПутьБезКавычек(СоответствиеПоказателейСтрокиСоединения.srvr) + "\"
				+ СтроковыеФункцииКлиентСервер.ПутьБезКавычек(СоответствиеПоказателейСтрокиСоединения.ref);
			ПутьКБазе = СтроковыеФункцииКлиентСервер.ОбернутьВКавычки(ПутьКБазе);
		...
		Если РежимЗапускаПодПользователем Тогда
			...
			ДанныеСохраненногоПароляПользователяИБ = ОбщегоНазначенияВызовСервера.ДанныеСохраненногоПароляПользователяИБ(
				Пользователь);
			ОбщегоНазначенияВызовСервера.УстановитьПарольПользователюИБ(Пользователь, ВременныйПароль);
...
Процедура ЗапуститьСеанс1СЗавершениеЗапуска(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияВызовСервера.ВосстановитьДанныеПользователяПослеЗапускаСеансаПодПользователем(
